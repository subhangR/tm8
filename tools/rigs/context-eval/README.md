...
- More isolation limits this run (D9 addendum), all next-run items in DESIGN.md §7 (f)-(h):
...
- The rig does NOT see subagent transcripts (`<transcript-dir>/<native-id>/subagents/agent-*.jsonl`). A lane that delegates undercounts `requests`, `usage`, `toolCalls`, reads and `costUsd`. First-request tokens, components, misses from the main thread and the rubric are unaffected. Seen on 2 lanes this run, both sonnet5 replica-01a0d742#1 (c3 01a0d982-d7a7: 36 subagent requests, 2.80M input-side tokens; c4 01a0d985-1dce: 37 requests, 3.93M). Schema 4 counts them (DESIGN.md §8.4).
