...
- (n) Lane worktrees are not left readable for later lanes (decision D12; designer msg 01a0d9af-57eb; C1 msg 01a0d9ae-e864). Every lane worktree sits under `<datadir>/worktrees/<project>/`, readable by every other lane on the node. In the first run two Haiku stress30 lanes (c1 01a0d9aa-829e, c4 01a0d995-9cef) could not find the withheld spec, READ a Sonnet sibling's worktree, copied `SINGLE_TRANSACTION_LIMIT` and the throw, and scored 1.00; both were set aside.
  - lanes.mjs moves a finished lane's worktree to `<datadir>/evidence/worktrees/<session>/` right after judging: success.mjs reads the committed head at judge time and never again, and remeasure needs only the manifest and the transcript. The row records `worktreeMovedTo`. Alternatively the worktree is made unreadable (`chmod -R a-rwx`) at lane end, and the worktrees root is not listable by lanes.
  - Two CONCURRENT lanes can still reach each other's worktree for those minutes, so detection stays. report.mjs `readSiblingWorktree [{path, sessionId}]` (in the report since this run) flags a Read/cat/grep/find under another row's worktree, in main and subagent transcripts. Such a row is set aside from success and from the D2/D6 gate rows, with reason 'copied from sibling worktree'.
  - §2 prints "silent + passed" (needle never opened AND all checks passed), the copy signature, so a copy cannot be confused with a lane that reasoned the rule out.
  - The SHARED GIT OBJECT STORE is a second copy channel that moving or locking worktrees cannot close (C1 msg 01a0d9b8-5e69). Every lane worktree shares the node's fixture-repo objects and refs, so `git log --all` + `git show <other lane's sha>:src/ledger.js` reads any earlier or concurrent lane's committed answer. c1's third haiku45/stress30 attempt, run alone after all worktrees were locked, copied this way. v3: each lane gets its OWN CLONE (`git clone --no-hardlinks` of fixture main into the lane dir), or the lane branch is deleted from the shared repo right after judging, so no other lane's commits are reachable. Detection this run: report.mjs `readSiblingCommit [{sha, sessionId}]` flags a git show/log/diff/cat-file/checkout of a commit that is not an ancestor of the lane's own branch (`tm8/<worktree id>`), attributed via `git branch --contains`. The row is set aside like readSiblingWorktree. `listedAllRefs` records `git log --all` / `branch -a` without setting a row aside.
  - Model-behaviour note for the consolidated read: when the spec was absent, Haiku went looking in sibling worktrees and the shared object store and copied (c1 x3 attempts, c4 x1), while Sonnet on the same absent-needle stress cells fetched the doc by id. It is NOT Haiku-only: c3's sonnet5/turn-date#1 PILOT lane (01a0d963-4f15) ran `git log --all` and `git show 50a54c7 -- src/ledger.js`, the commit of the concurrently running haiku45/turn-date#1 (01a0d962-49da), before committing its own.
