...
describe('W1.C generated catalog and reachability foundations', () => {
  it('derives exact current catalog, router, status, method, and kind accounting', async () => {
    ...
    expect(manifest.catalog).toEqual({
      ...
      total: 240, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs (task 01a0d350). // +1 entities.commands.tick (bug 01a0d2f1). +1 events.changes (change feed step 3).
      v1: 238, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs. // +1 entities.commands.tick. +1 events.changes (change feed step 3).
      reserved: 2,
      http: 238, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs. // +1 entities.commands.tick. +1 events.changes (change feed step 3).
      ws: 1,
      registerableV1Http: 236, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs. // +1 entities.commands.tick. +1 events.changes (change feed step 3).
      methods: { GET: 79, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ POST: 114, PATCH: 16, DELETE: 16, PUT: 13, WS: 2 }, // +1 events.changes (change feed step 3). // SC-8: GET +1 shares.list, POST +1 space.share. MEASURED.
      kinds: { read: 83, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ command: 155, stream: 2 }, // +1 events.changes (change feed step 3). // SC-8: read +1, command +1. MEASURED.
      uniqueNames: 240, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs. // +1 entities.commands.tick. +1 events.changes (change feed step 3).
      uniqueBindings: 239, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs. // +1 entities.commands.tick. +1 events.changes (change feed step 3).
    });
    ...
    expect(manifest.routes.http).toHaveLength(238); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). +3 (Jev lane K). MEASURED.
  ...
  it('keeps implementation accounting honest at 28 handlers and 36 actual input bindings', async () => {
    ...
    expect(manifest.serverRegistries.unimplementedV1Http).toBe(208); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // 180 -> 190 (SC-3). 177 -> 180 (Jev lane K).
  ...
  it('reproduces the frozen W1 registry boundary from an immutable checked-in snapshot', async () => {
    ...
    expect(manifest.serverRegistries).toEqual({
      ...
      unimplementedV1Http: 208, /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs (task 01a0d350). // +1 entities.commands.tick (bug 01a0d2f1). 180 -> 190 (SC-3): registerableV1Http 218 minus 28. 177 -> 180 (Jev lane K). // +1 events.changes (change feed step 3).
    });
  });
  ...
  it('keeps prompt internal-only, reserved help honest, and WS durability unclaimed', async () => {
    ...
    // 2026-09-23 (filesystem skills INTEGRATED WITH main): skills.scan/list/show/preview, all v1 HTTP (3 GET/read, 1 POST/command). MEASURED on the merged tree.
    expect(manifest.help.operations).toHaveLength(240); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). // +3 service keys (Jev lane K). // +1 launch.suggest (Jev lane F). MEASURED.
...
describe('W2.C01 current mounted registry inventory', () => {
  it('proves the exact I03 tranche-v3 handler and input-schema boundary from current source', async () => {
    ...
    expect(handlers.facade).toHaveLength(221); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ // +10 space/node credentials (SC-3). +3 service keys (Jev lane K). MEASURED.
    ...
    expect(handlers.all).toHaveLength(236); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). +3 (Jev lane K). MEASURED.
    ...
    expect(createHash('sha256').update(JSON.stringify(handlers.all)).digest('hex'))
      ...
      .toBe('4230bafc8bbd173afd9fd4baa3ae9d7f8a7c0e208e110f75c945cb4c2c8aa5a6'); // Re-measured (SC-8): credentials.space.share + credentials.shares.list join; computed by CALLING readHandlerSourceInventory. // Re-measured (Forms W1): the thirteen forms.* handlers join; read from the failing run's Received line.
    ...
    expect(inputSchemas.bound).toHaveLength(147) /* +1 credentials.space.share binds (SC-8). MEASURED. */ /* +10 forms.* command schemas (Forms W1). MEASURED. */ // +1: entities.commands.tick binds TickCriteriaInputSchema (bug 01a0d2f1). +7: the SC-3 space/node credential commands bind. +2: service-key put and delete bind (Jev lane K).
    ...
    expect(registerableV1Http).toHaveLength(236); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). +3 (Jev lane K). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
