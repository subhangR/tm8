...
describe('W1.C generated catalog and reachability foundations', () => {
  it('derives exact current catalog, router, status, method, and kind accounting', async () => {
    ...
    expect(manifest.catalog).toEqual({
      ...
      // 2026-08-13 (first-run claim): auth.claim (POST command) +
      // auth.claim.status (GET read).
      total: 161,
      v1: 159,
      reserved: 2,
      http: 160,
      ...
      methods: { GET: 59, POST: 74, PATCH: 10, DELETE: 10, PUT: 7, WS: 1 },
      kinds: { read: 62, command: 98, stream: 1 },
      uniqueNames: 161,
      uniqueBindings: 161,
    });
    ...
    expect(manifest.routes.http).toHaveLength(160);
  ...
  it('keeps prompt internal-only, reserved help honest, and WS durability unclaimed', async () => {
    ...
    expect(manifest.help.operations).toHaveLength(161);
...
describe('W2.C01 current mounted registry inventory', () => {
  it('proves the exact I03 tranche-v3 handler and input-schema boundary from current source', async () => {
    ...
    // 90 -> 91: auth.claim binds a body; auth.claim.status is a read.
    expect(inputSchemas.bound).toHaveLength(91);
    ...
    expect(registerableV1Http).toHaveLength(158);
