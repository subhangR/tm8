...
describe('W1.C generated catalog and reachability foundations', () => {
  it('derives exact current catalog, router, status, method, and kind accounting', async () => {
    ...
    expect(manifest.catalog).toEqual({
      ...
      methods: { GET: 49, POST: 56, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 },
      kinds: { read: 52, command: 79, stream: 1 },
      uniqueNames: 132,
      uniqueBindings: 132,
    });
    ...
    expect(manifest.routes.http).toHaveLength(131);
  ...
  it('keeps implementation accounting honest at 28 handlers and 36 actual input bindings', async () => {
    ...
    expect(manifest.serverRegistries.unimplementedV1Http).toBe(101);
  ...
  it('keeps prompt internal-only, reserved help honest, and WS durability unclaimed', async () => {
    ...
    expect(manifest.help.operations).toHaveLength(132);
...
describe('W2.C01 current mounted registry inventory', () => {
  it('proves the exact I03 tranche-v3 handler and input-schema boundary from current source', async () => {
    ...
    expect(handlers.facade).toHaveLength(119);
    // Tranche-v7 = tranche-v6 plus exactly THREE facade handlers, all in the
    // per-user credential lane: gitCredentials.set/status/delete. They mount
    // unconditionally — the credential store needs no blob storage.
    //
    ...
    expect(handlers.all).toHaveLength(129);
    ...
    expect(createHash('sha256').update(JSON.stringify(handlers.all)).digest('hex'))
      .toBe('3304f620a50384072131930647ba27b707e62fa939ed82a5cb3314192d3f717c');
    ...
    expect(registerableV1Http).toHaveLength(129);
