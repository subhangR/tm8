...
describe('W1.C generated catalog and reachability foundations', () => {
  it('derives exact current catalog, router, status, method, and kind accounting', async () => {
    ...
    expect(manifest.catalog).toEqual({
      ...
      http: 140,
      ws: 1,
      registerableV1Http: 138,
      methods: { GET: 53, POST: 61, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 },
      kinds: { read: 56, command: 84, stream: 1 },
      uniqueNames: 141,
      uniqueBindings: 141,
    });
    ...
    expect(manifest.routes.http).toHaveLength(140);
  ...
  it('keeps implementation accounting honest at 28 handlers and 36 actual input bindings', async () => {
    ...
    // 138 current registerable v1 HTTP ops minus the 28 W1-implemented.
    expect(manifest.serverRegistries.unimplementedV1Http).toBe(110);
  ...
  it('reproduces the frozen W1 registry boundary from an immutable checked-in snapshot', async () => {
    ...
    expect(manifest.serverRegistries).toEqual({
      ...
      // what is mounted today. Read out of the failure, which said 110.
      unimplementedV1Http: 110,
    });
  });
  ...
  it('keeps prompt internal-only, reserved help honest, and WS durability unclaimed', async () => {
    ...
    expect(manifest.help.operations).toHaveLength(141);
...
describe('W2.C01 current mounted registry inventory', () => {
  it('proves the exact I03 tranche-v3 handler and input-schema boundary from current source', async () => {
    ...
    // projects.folderUploads.init/complete/abort add three, registered in the
    // same w2 project-files module because they drive the same blob store.
    expect(handlers.all).toHaveLength(138);
    ...
    expect(createHash('sha256').update(JSON.stringify(handlers.all)).digest('hex'))
      ...
      // working, not a pin to route around. It moved again for the three
      // `projects.folderUploads.*` rows registered in that same module.
      .toBe('bb8739550af0df410bd2cefc52de63a78f4bc823efa2da05baa7e58c700981f6');
    ...
    expect(registerableV1Http).toHaveLength(138);
