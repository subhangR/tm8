...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  // 128 -> 130 (2026-08-09): `files.browse` + `files.read`, two GET reads —
  // the Files browser over the node's real filesystem (FILES-DESIGN §5.1).
  // Every number below is MEASURED off the built catalog, never derived as
  // previous-plus-two: a pin that was already stale would otherwise stay stale.
  ...
  assertEqual(methods, { GET: 50, POST: 54, PATCH: 10, DELETE: 8, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 53, command: 76, stream: 1 }, 'kind accounting');
