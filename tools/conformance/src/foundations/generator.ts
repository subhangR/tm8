...
function nounForOperation(operation: OperationName): string {
  ...
  switch (family) {
    ...
    case 'gitCredentials': return 'git-credential';
...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 132, 'catalog total');
  assertEqual(V1_OPERATIONS.length, 130, 'v1 total');
  ...
  assertEqual(methods, { GET: 49, POST: 56, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 52, command: 79, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 131, 'server router HTTP total');
