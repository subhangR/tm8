...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 256, 'catalog total'); // +3 spaces.leave, spaces.members.remove, accounts.disable (G6, 232). MEASURED. // +2 spaces.chatDefaults.get/set (entity chat G). // +1 launch.defaults (I9b). MEASURED. // +2 auth.sessions.list/revoke (W4). MEASURED.
  ...
  assertEqual(V1_OPERATIONS.length, 254, 'v1 total'); // +3 spaces.leave, spaces.members.remove, accounts.disable (G6, 232). MEASURED. // +2 spaces.chatDefaults.get/set (entity chat G). // +1 launch.defaults (I9b). MEASURED. // +2 auth.sessions.list/revoke (W4). MEASURED.
  ...
  assertEqual(methods, { GET: 85, /* +1 node.metrics.get (status strip). MEASURED. */ POST: 121, PATCH: 16, DELETE: 17, PUT: 15, WS: 2 }, 'method accounting'); // W11: GET +2, POST +2 (projects.link stays, decision 29). // POST +3 G6 leave/remove/disable. // GET +1, PUT +1 spaces.chatDefaults.get/set. // GET +1 auth.sessions.list, POST +1 auth.sessions.revoke (W4). MEASURED.
  ...
  assertEqual(kinds, { read: 89, /* +1 node.metrics.get (status strip). MEASURED. */ command: 165, stream: 2 }, 'kind accounting'); // W11: read +2, command +2 (projects.link stays, decision 29). // command +3 G6 leave/remove/disable. // read +1, command +1 spaces.chatDefaults.get/set. // read +1 auth.sessions.list, command +1 auth.sessions.revoke (W4). MEASURED.
  ...
  assertEqual(router.http.length, 254, 'server router HTTP total'); /* +1 node.metrics.get (status strip). MEASURED. */ // +3 G6 leave/remove/disable. MEASURED. // +2 spaces.chatDefaults.get/set (entity chat G). // +1 launch.defaults (I9b). MEASURED. // +2 auth.sessions.list/revoke (W4). MEASURED.
