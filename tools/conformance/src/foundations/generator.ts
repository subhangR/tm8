...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 139, 'catalog total');
  ...
  assertEqual(methods, { GET: 53, POST: 59, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 56, command: 82, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 138, 'server router HTTP total');
