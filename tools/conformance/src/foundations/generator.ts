...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 125, 'catalog total');
  assertEqual(V1_OPERATIONS.length, 123, 'v1 total');
  ...
  assertEqual(methods, { GET: 45, POST: 54, PATCH: 10, DELETE: 8, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 48, command: 76, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 124, 'server router HTTP total');
