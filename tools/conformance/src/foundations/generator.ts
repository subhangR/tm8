...
function nounForOperation(operation: OperationName): string {
  ...
  switch (family) {
    ...
    // `spaceLinks.*` (W6, 243/244, `cmd: null`): `space-link`, as NOUN_BY_FAMILY says.
...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 263, 'catalog total'); /* +7 spaceLinks.* (W6, 243/244). MEASURED. */ // +3 spaces.leave, spaces.members.remove, accounts.disable (G6, 232). MEASURED. // +2 spaces.chatDefaults.get/set (entity chat G). // +1 launch.defaults (I9b). MEASURED. // +2 auth.sessions.list/revoke (W4). MEASURED.
  ...
  assertEqual(V1_OPERATIONS.length, 261, 'v1 total'); /* +7 spaceLinks.* (W6, 243/244). MEASURED. */ // +3 spaces.leave, spaces.members.remove, accounts.disable (G6, 232). MEASURED. // +2 spaces.chatDefaults.get/set (entity chat G). // +1 launch.defaults (I9b). MEASURED. // +2 auth.sessions.list/revoke (W4). MEASURED.
  ...
  assertEqual(methods, { GET: 86, /* GET +1, POST +5, PATCH +1 spaceLinks.* (W6). MEASURED. */ /* +1 node.metrics.get (status strip). MEASURED. */ POST: 126, PATCH: 17, DELETE: 17, PUT: 15, WS: 2 }, 'method accounting'); // W11: GET +2, POST +2 (projects.link stays, decision 29). // POST +3 G6 leave/remove/disable. // GET +1, PUT +1 spaces.chatDefaults.get/set. // GET +1 auth.sessions.list, POST +1 auth.sessions.revoke (W4). MEASURED.
  ...
  assertEqual(kinds, { read: 90, /* read +1, command +6 spaceLinks.* (W6). MEASURED. */ /* +1 node.metrics.get (status strip). MEASURED. */ command: 171, stream: 2 }, 'kind accounting'); // W11: read +2, command +2 (projects.link stays, decision 29). // command +3 G6 leave/remove/disable. // read +1, command +1 spaces.chatDefaults.get/set. // read +1 auth.sessions.list, command +1 auth.sessions.revoke (W4). MEASURED.
  ...
  assertEqual(router.http.length, 261, 'server router HTTP total'); /* +7 spaceLinks.* (W6). MEASURED. */ /* +1 node.metrics.get (status strip). MEASURED. */ // +3 G6 leave/remove/disable. MEASURED. // +2 spaces.chatDefaults.get/set (entity chat G). // +1 launch.defaults (I9b). MEASURED. // +2 auth.sessions.list/revoke (W4). MEASURED.
