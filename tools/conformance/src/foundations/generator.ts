...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  // 137 -> 146, MEASURED on feat/tier2-cherry-branch-stash (the 137 pin was
  // already stale on feat/git-ui-wave, which appended without re-pinning):
  // execution.gitStatus/gitDiff (2 GET/read) + execution.gitCheckpoint/
  // gitRollback/gitCommit/gitMerge (4 POST/command) from the git-ui wave,
  // then Tier 2 completion's execution.gitCherryPick/gitBranch/gitStash
  // (3 POST/command).
