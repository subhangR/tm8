...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 102, 'catalog total');
  assertEqual(V1_OPERATIONS.length, 100, 'v1 total');
  ...
  assertEqual(methods, { GET: 37, POST: 41, PATCH: 9, DELETE: 7, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 40, command: 61, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 101, 'server router HTTP total');
