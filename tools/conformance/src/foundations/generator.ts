...
function nounForOperation(operation: OperationName): string {
  ...
  switch (family) {
    ...
    // The control plane, deliberately its own noun rather than a corner of
    // `auth`: authentication proves who you already are, while `user` is about
    // a person existing on this node at all — account, Space, home, capabilities.
...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  // 142 -> 146 (2026-08-11, control plane): users.create, users.list and the
  // two capability commands. `users.create` provisions a whole user — account,
  // their own Space, and the record of their home — where `auth.signup` wrote
  // an account and stopped.
  assertEqual(names.length, 146, 'catalog total');
  assertEqual(V1_OPERATIONS.length, 144, 'v1 total');
  ...
  assertEqual(methods, { GET: 54, POST: 65, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 57, command: 88, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 145, 'server router HTTP total');
