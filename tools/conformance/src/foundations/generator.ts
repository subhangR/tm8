...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 120, 'catalog total');
  assertEqual(V1_OPERATIONS.length, 118, 'v1 total');
  ...
  assertEqual(methods, { GET: 44, POST: 50, PATCH: 10, DELETE: 8, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 47, command: 72, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 119, 'server router HTTP total');
