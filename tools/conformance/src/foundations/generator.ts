...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(names.length, 199, 'catalog total');
  ...
  assertEqual(V1_OPERATIONS.length, 197, 'v1 total');
  ...
  assertEqual(methods, { GET: 65, POST: 100, PATCH: 12, DELETE: 12, PUT: 8, WS: 2 }, 'method accounting');
  ...
  assertEqual(kinds, { read: 69, command: 128, stream: 2 }, 'kind accounting');
  ...
  assertEqual(router.http.length, 197, 'server router HTTP total');
