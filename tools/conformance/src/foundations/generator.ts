...
export async function buildW1ConformanceManifest(): Promise<W1ConformanceManifest> {
  ...
  assertEqual(methods, { GET: 53, POST: 62, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 }, 'method accounting');
  assertEqual(kinds, { read: 56, command: 85, stream: 1 }, 'kind accounting');
  assertEqual(router.http.length, 141, 'server router HTTP total');
