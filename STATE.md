...
- **DO NOT port "mount-only-active" xterm** (unmount inactive terminals, rebuild a fresh xterm + full-ring-replay on switch). It LEAKS — retained old instances, heap climb, thousands of detached DOM. **Keep-mounted + socket-SUSPEND is the correct model** — exactly the visibility driver Draco already ported (terminals constructed ONCE under a stable key; offscreen sockets suspended; only hidden EXITED terminals unmount). When we build the Option-C multi-terminal dock (Phase 2), it MUST use keep-mounted+suspend, never mount-only-active. tm8's current single-terminal-per-view with clean `term.dispose()` on unmount is fine for Wave 1.
- **Active-only cursor blink** — Draco already did this (cursorBlink = active && setting). Don't let a settings effect re-enable blink on all mounted terminals (N repaint timers).
- **Bound the session broadcast payload server-side.** maestro's `session:updated` re-broadcast the full ever-growing session ~2×/sec → per-message parse cost climbs over a session's life. tm8 mostly avoids this by design (events.poll with per-space seq cursors = the "timeline_appended delta" variant, not full-resend) — but VERIFY tm8 never broadcasts/returns unbounded work_session timeline/message history; cap if found. Wave 2 perf-check.
- **Audio element leak** (maestro's soundManager cloneNode'd an <audio> per event, never released it). CHECK the transplanted collab-v2 soundManager for the same; release on ended/error/AND play().catch. Wave 2 perf-check.
- Method note: "reload fixes it" == client heap/state; DevTools Detached-elements + 2-snapshot heap Comparison were decisive; verify against the SERVED prod bundle (no Vite/HMR/StrictMode double-invoke), not dev.

Also recorded — transplant heuristic (Draco): when both repos run lifted code and only one looks broken, suspect the INSTRUMENT before the code (the char-drop was a diagnostic regex flattening TUI reflow; `writePromptToEntry` is byte-identical maestro↔tm8).

## UI — full gap audit done (docs/ui-audit/UI-GAP-AUDIT.md), Wave 0 build IN FLIGHT (2026-07-25)

Atlas (opus-5 UI lead, sess_1784982701236_2nq0j85d9) delivered a 712-line audit + 5 research reports. Verdict: graph half of the UI is real (Tasks/Team/Home/Docs/board/Z3 panel alive on real data); execution half doesn't exist as a product. Key: no sessions surface (a running agent is unreachable on reload — F1); sessions LIST needs no contract change (collections.query over kinds:['work_session'] returns 200 today — F2); Settings white-screens the whole app + NO error boundary (F3); only 5 capabilities need contract amendments — contract isn't the bottleneck (F4); all ~534 UI tests run on MockFacade so none see the real path (F5); honest-degradation gating (real/capabilities.ts) never wired (F6). UX brief vendored (F7). Full roadmap = 6 waves in the doc. **Wave 0 DONE & committed (2c37881), Vega-verified** (typecheck clean, ui 887/execution 35, sessions nav+list render live 0 errors, reattach captured on isolated echo stack). Delivered: ErrorBoundary (per-view + per-panel), Settings root-cause fix (mis-shaped getSettings behind an `as unknown` cast — cast deleted) + rail entry, registryFor fallback, **sessions rail+list+route+reattach** (state in the ROUTE not modal; SpawnDialog navigates), deep-link round-trip, TmClient delete/put, grantPoints, events.poll cursor, Z3 clip, first REAL-PATH integration test. Two Vega-approved architecture calls: (1) Sessions screen injected via a new optional `extraViews` prop on CollabV2App (tm8 TOUCH #2, same seam shape as installTm8Kinds — keeps tm8 views on the tm8 side; note for any future re-sync); (2) `.cv2-panel` class-name collision (entity.css + shell.css both define it) — symptom fixed, rename DEFERRED (touches both subsystems + tests). Open from F6: a capability signal on the facade seam itself (Settings got a local `unavailable` field; the general fix is still open). **Original Atlas Wave 0 leading-it list (for reference):**

**User chose Wave 0 build-only.** Atlas led it: (1) error boundary + Settings crash fix + registryFor fallback, (2) **sessions nav + list + reattach** (durable session state, terminal reattaches over the WS), (3) correctness batch (deep-link codec, grantPoints, TmClient delete/put, events.poll cursor, Z3 clip, stale PTY copy), (4) a REAL-PATH integration test so this can't hide again. DEFERRED from Wave 0: project trust-default (entangled with the missing confirmUntrusted contract field + no project-creation UI — Wave 3); all design-dependent surfaces (sessions IA polish, terminal chrome, prompt composer, auth, projects/spaces/custom-kind UIs — Waves 1/3, design proposals to come). Waves 0+2 (build) run parallel to 1+3 (design). Contract amendments gate 3-5 — draft early.
...
- ~~Prompt delivery DROPS CHARACTERS~~ **RETRACTED — false alarm (Draco, 2026-07-25).** execution.prompt delivers BYTE-EXACT end-to-end (5 new fidelity tests, 35 green); the "atualy"/"t8" was Claude's TUI reflow/redraw being read as output, not lost bytes. **Slice 4 directive delivery is NOT blocked.** Fidelity tests kept.
