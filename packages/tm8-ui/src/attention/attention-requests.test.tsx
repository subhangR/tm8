...
describe('attention history model', () => {
  ...
  it('names the actor and the outcome, and calls a dismissal DECLINED not resolved', () => {
    ...
    expect(settlementLine(req({ id: '1', status: 'resolved', ...base }), NOW)).toBe('Resolved by Bo · 24h ago');
    expect(settlementLine(req({ id: '2', status: 'dismissed', ...base }), NOW)).toBe('Declined by Bo · 24h ago');
  });
});
...
describe('AttentionRequests', () => {
  ...
  it('shows SETTLED rows, which the badge can never show', async () => {
    ...
    expect(getByText('Resolved by Subhang · 24h ago')).toBeTruthy();
