...
export const VIEW_ART = {
  ...
  /**
   * A FOLDER, deliberately NOT `KIND_ART.file`'s page glyph. The Files view
   * browses a directory tree on DISK; the `file` kind lists file ENTITIES,
   * which are references to files whose truth lives elsewhere. Two different
   * promises, so two different silhouettes — sharing one would tell the reader
   * the rail row and the kind list are the same place.
   */
  files: [
