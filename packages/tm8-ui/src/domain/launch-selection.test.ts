...
describe('the 240 ceiling', () => {
  ...
  it('composeLaunchSelection never sends an over-ceiling set, whatever the caller built', () => {
    const huge = { send: many(SPAWN_SELECTION_GROUP_LIMIT + 1, 'x').map(id) };
    expect(composeLaunchSelection({ ...OMITTED, skills: huge })).toEqual({
      selectionReasons: { memories: 'not-asked', skills: 'not-asked', references: 'not-asked' },
    });
  });
});

describe('composeLaunchSelection — per-group send', () => {
  it('no group edited: no `selection`, and every group says not-asked', () => {
    expect(composeLaunchSelection(OMITTED)).toEqual({
      selectionReasons: { memories: 'not-asked', skills: 'not-asked', references: 'not-asked' },
    });
  });

  it('only the edited group is sent; the others keep their defaults with a reason', () => {
    const fields = composeLaunchSelection({ ...OMITTED, references: { send: [id('r1')] } });
  ...
  it('a group Jev was asked about and nobody applied says jev-pending while an edited one goes', () => {
    const fields = composeLaunchSelection({ ...OMITTED, skills: { send: [id('s1')] } }, { memories: 'jev-pending' });
  ...
  it('a failed Jev group explains a defaulted group, never an edited one', () => {
