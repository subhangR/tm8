...
export interface JevAppliedModel {
  ...
  /** The teammate the launch ran as when the model was applied — how "replaced" says who replaced it. */
  teammateId: string | null;
}

/** Why an applied model or teammate no longer describes the launch; null while it still does. */
export interface JevReplaced {
  model: string | null;
...
export interface JevSuggestions {
  ...
  toggle(group: JevEntityGroup, id: string): string | null;
  ...
  /**
   * The model and teammate entries' currentness (like a group's
   * `appliedIsCurrent`): a person's own pick after an Apply replaces it, and
...
/** The launch carries this applied model. A null applied effort means "the model's default", so any effort matches it. */
function carriesModel(a: JevModelChoice | null | undefined, v: JevModelChoice): boolean {
  return Boolean(a) && a?.model === v.model && a?.agentToolId === v.agentToolId
    && (v.reasoningEffort === null || a?.reasoningEffort === v.reasoningEffort);
}

export const JEV_CHANGED_BY_HAND = 'Applied, then changed by hand.';
export const JEV_REPLACED_UNDO_REASON = 'Replaced since it was applied — Re-apply it, or leave the launch as it is.';
export const jevModelReplacedBy = (teammate: string) => `Applied, then replaced by ${teammate}’s default model.`;
...
export function useJevSuggestions(args: {
  ...
  const toggle = useCallback((group: JevEntityGroup, id: string): string | null => {
    ...
    if (!current.includes(id as EntityId) && current.length >= SPAWN_SELECTION_GROUP_LIMIT) {
      setTickRefusal({ group, id, reason: TICK_CEILING_REASON });
  ...
  /* CURRENTNESS of the model and teammate entries. A person's own pick after
     an Apply (the card's teammate picker re-seeds the model; a hand model
     pick) replaces the entry: the ledger must not say "Applied" for what the
     launch no longer carries. The entry stays, so Re-apply can restore it. */
  const teammateTitle = (id: string | null | undefined): string => {
    const hit = groups.teammates.status === 'ok' ? groups.teammates.value.items.find((item) => item.entityId === id) : undefined;
    return hit?.title ?? 'the new teammate';
  };
  const replaced: JevReplaced = {
    model: applied.model && host?.model !== undefined && !carriesModel(host.model, applied.model.value)
      ? (applied.model.teammateId !== (teammateId ?? null) ? jevModelReplacedBy(teammateTitle(teammateId)) : JEV_CHANGED_BY_HAND)
      : null,
    teammate: applied.teammate && applied.teammate.teamMemberId !== teammateId ? JEV_CHANGED_BY_HAND : null,
  };
  ...
  const applyTeammate = useCallback((teamMemberId?: string): string | null => {
    ...
    /* TWO JEV APPLIES COMPOSE in either order: the surface's teammate setter
       re-seeds the model from the new persona, so a model Jev applied that the
       launch still carries is applied again after it. A teammate picked BY
       HAND is not a Jev action and does not do this — the model row then says
       it was replaced, with Re-apply. */
    const { applied: ledgerNow, replaced: replacedNow } = view.current;
    const keepModel = ledgerNow.model && !replacedNow.model && h.setModel ? ledgerNow.model.value : null;
    if (keepModel) h.setModel?.(keepModel);
    setApplied((ledger) => ({
      ...ledger,
      teammate: { at: now(), teamMemberId: target, previous: ledger.teammate?.previous ?? current ?? null },
      ...(keepModel && ledger.model ? { model: { ...ledger.model, teammateId: target } } : {}),
    }));
    return null;
  }, []);
  ...
  const applyModel = useCallback((): string | null => {
    ...
    h.setModel(value);
    const teammateAt = live.current.teammateId ?? null;
    setApplied((ledger) => ({ ...ledger, model: { at: now(), value, previous: ledger.model?.previous ?? previous, teammateId: teammateAt } }));
    return null;
  }, []);
  ...
  const applyAll = useCallback((): JevApplyReport => {
    ...
    /* THE TEAMMATE FIRST: a surface's teammate setter re-seeds the model from
       the new persona, so a model applied before it would be silently undone
       while the ledger still said "Applied" (found in the lane-D walkthrough).
       A teammate Apply that CHANGES the teammate also makes the three groups'
