...
export const GOVERNANCE_REASONS = {
   ...
   * not (the CLI `tm8 project add` carries `SpaceProjectCreateInput`).
   */
  linkProject: {
    cause: 'Linking a project can’t be done from this screen yet',
    remedy:
      'this settings seam carries no link executor — link from the CLI: `tm8 project create <name> --working-dir <abs-path>` then `tm8 project add <folder-id> --space <space>` (adding a project while creating a space links one too)',
  },
...
export const GOVERNANCE_GAPS: readonly GovernanceGap[] = [
  { id: 'GG1', need: 'link a project to a space', seamToday: 'no link executor in this screen’s seam (the CLI `tm8 project add` and the Add-space onboarding do carry it)', reason: 'linkProject' },
