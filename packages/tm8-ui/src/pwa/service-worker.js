 ...
 *
 * `/ui-2.0/` is a whole second application bundle, not a route of this one.
 ...
 * when the network fails, so an offline navigation to `/ui-2.0/` would boot
 * the 1.0 shell at the 2.0 address — the switch would appear to do nothing.
 * Its assets would also be cached under this worker's key while its own build
 * hashes are unknown here, so a 2.0 redeploy could not evict them.
 */
function isOtherUi(url) {
  return url.pathname === '/ui-2.0' || url.pathname.startsWith('/ui-2.0/');
}
