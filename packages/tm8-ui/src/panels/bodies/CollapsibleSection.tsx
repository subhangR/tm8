...
export interface CollapsibleSectionProps {
   ...
   *
   * A LITERAL `0` IS NOT DRAWN. A section with no children used to render a
   * lone right-aligned `0` against an otherwise empty row, which reads as a
   * rendering failure rather than as a fact. The section already states its own
   * emptiness — through its body, or by leaving the flow entirely when `empty`
   * is true — so the badge simply stands down. `undefined` and `0` therefore
   * render identically.
   */
  count?: ReactNode;
