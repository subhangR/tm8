...
export function SkillPreview({ load, teamMemberId, projectId, agentTool }: { load?: (input: Parameters<SkillPort['preview']>[1]) => Promise<SkillPreviewResult>; teamMemberId: string; projectId?: string; agentTool?: string }) {
  ...
  return <section className="ls__section"><div className="ls__eyebrow">SKILLS</div>{error ? <p role="alert">{error}</p> : skills ? <><SkillGroups skills={skills} />{skills.rows.filter(r => r.missing).map(r => <p key={r.entityId}>Missing: {r.name} · {r.sourcePath}</p>)}</> : <p>{load ? 'Loading skill preview…' : 'Skill preview unavailable'}</p>}</section>;
}
