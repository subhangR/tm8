...
export function SkillPreview({ load, teamMemberId, projectId, agentTool, children, bare = false }: { load?: (input: Parameters<SkillPort['preview']>[1]) => Promise<SkillPreviewResult>; teamMemberId: string; projectId?: string; agentTool?: string; children?: ReactNode; /** Without its own section and SKILLS heading — nested under the Skills group. */ bare?: boolean }) {
  ...
  const body = <>{error ? <p role="alert">{error}</p> : skills ? <><SkillGroups skills={skills} />{skills.rows.filter(r => r.missing).map(r => <p key={r.entityId}>Missing: {r.name} · {r.sourcePath}</p>)}</> : <p>{load ? 'Loading skill preview…' : 'Skill preview unavailable'}</p>}{children}</>;
  return bare ? <div className="ls__section">{body}</div> : <section className="ls__section"><div className="ls__eyebrow">SKILLS</div>{body}</section>;
}
