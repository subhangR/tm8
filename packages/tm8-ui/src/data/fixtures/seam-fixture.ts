...
>>>>>>> dca54ecd (WIP: Phase 6 — CLI + UI tranche (rescued from dead session 01a01558))
  ...
  const seam: FixtureSeam = {
     ...
     * Do initial. Zero transition rows — empty means "the ruled category-level
     ...
     * Phase 6 (migration 153) dropped `public.task_workflows` whole, so there
     ...
     * yet curate — and that absence is honest, not a stub.
     */
    ...
    async entityKinds(spaceId): Promise<EntityKindDef[]> {
      ...
      return clone([...custom.entries()].map(([kind, sample]): EntityKindDef => {
        ...
        /*
         * 153's four new members, answered the way a real node answers them
         * for a kind this fixture INFERRED from entity rows rather than from a
         * `entity_kinds` table it does not have.
         *
         * `baseKind: null` and null labels are the HONEST answers here: a kind
        ...
        return {
          ...
          // title-cased. Before 153 the client had nowhere to get this from and
...
=======
        ...
        // 153) dropped `public.task_workflows`, and with it the trigger, so a
...
=======
        ...
        // in phase 6 (migration 153). See the `updateTask` note above.
