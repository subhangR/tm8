...
/**
 * Two board rows, one live and one revoked.
 *
 * The roles come from `inviteRoles()` rather than being written as `'admin'`
 * and `'member'`, and the reason is §15.2 rather than taste: the
 * least-privileged role and the member KIND are the same string, so a literal
 * here is indistinguishable from a kind-literal violation to this lane's own
 * guard (`no-kind-literals.test.ts`) — and to a reader.
 */
const BOARD_INVITES: SpaceInviteView[] = (() => {
  const [privileged, plain] = [inviteRoles()[0]!, inviteRoles()[1] ?? inviteRoles()[0]!];
  return [
    { id: 'inv-a', code: 'inv_7f3c9b', role: plain as SpaceInviteView['role'],
      maxUses: 5, uses: 2, expiresAt: null, revoked: false },
    { id: 'inv-b', code: 'inv_30bb41', role: privileged as SpaceInviteView['role'],
      maxUses: 1, uses: 0, expiresAt: null, revoked: true },
  ];
})();
...
export function SettingsBoard({ now = 0 }: { now?: number }) {
  ...
  return (
    <div className="set-board">
      {/* No `onRoleChange`: the board draws the READ-ONLY viewer, where every
          role renders locked with its reason. The live select needs a real port
          and belongs on the shell, not on a specimen canvas. */}
      <Both label="T2-1b · Members & roles (read-only viewer)">
        <div className="set-panel">
          <MembersSection members={members} identity={SPECIMEN_IDENTITY} onInvite={() => {}} />
        </div>
      </Both>

      <Both label="T2-1b · Members & roles (admin — live selects)">
        <div className="set-panel">
          <MembersSection
            members={members}
      ...
      {/* Was "product — no capability" until 109. The capability was always
          there (007's RPCs); it was the seam that had no verb. This frame draws
          the UNREAD list, which is what a plain member gets because
          `spaces.invites.list` is admin-only. */}
      <Both label="T2-1c · Invites (unread — admin-only read)">
        <div className="set-panel" style={{ width: 262 }}>
          <InvitesPanel />
        </div>
      </Both>

      <Both label="T2-1c · Invites (live rows, wired)">
        <div className="set-panel" style={{ width: 262 }}>
          <InvitesPanel
            origin="https://tm8.example"
