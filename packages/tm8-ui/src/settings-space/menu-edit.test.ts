...
describe('add', () => {
  it('offers exactly the view refs the rowed base leaves free', () => {
    ...
    // …and 'codebrain' joined it 2026-09-01 (migration 173) in exactly the
    // 'craft' shape, then LEFT again 2026-09-15 (migration 186) when #610
    // deleted the package holding its screen. Nothing here was hardcoded into
    // moving in either direction — `availableViewRefs` is VIEW_PRESENTATION
    // minus what the draft places, which is why widening the union moved this
    // list on its own and narrowing it moved the list back.
    expect(availableViewRefs(startDraft(BASE))).toEqual([
      'feed', 'inbox', 'channels', 'craft', 'help',
    ]);
  });
