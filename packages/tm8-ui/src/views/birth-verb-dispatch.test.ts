...
import { NEW_CONTAINER_ACTIONS } from './useNewContainerSheet';
...
describe('birth verbs and the dispatchers behind them', () => {
  it('names a quickStart only where a dispatcher can perform it', () => {
    ...
    // `rootBirthAction`. Three kinds declare one today — work_session
    // (`start-terminal`), chat (`chat-about`) and container
    // (`new-container`); the last two are exactly the ones that were inert.
  ...
  it('covers `container` too — the containers lane hit this arm independently', () => {
    // Two lanes reached the same defect within a day, which is the argument
    // for the assertion above being a LOOP over the registry rather than a
    // list of the kinds someone remembered.
    expect(rootBirthAction('container')).toBe('new-container');
    expect(NEW_CONTAINER_ACTIONS).toContain('new-container');
    expect(SESSION_START_ACTIONS).not.toContain('new-container');
  });
});
