...
export function useGateData(options: GateOptions): GateData {
  ...
  const data = useMemo<GateData & { pull: (id: string) => void }>(
    ...
    [ready, spaceId, spaces, members, viewerActor, menu, connection, bootError, authRequired, liveIds, livenessOf, rowsFor, boardFor, countsFor, refreshCounts, detailOf, refetchDetail, connectionsOf, activity, messagePulses, graph, linkedPullRequestsOf, launch, ensureKind, selectSpace, acceptSpace, spawn, postAndRefresh, messagesByAnchor, reconcileCommand, seam, domain, pull],
  );
