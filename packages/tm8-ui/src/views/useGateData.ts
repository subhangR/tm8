...
export function useGateData(options: GateOptions): GateData {
  ...
  const hydrate = useCallback(
    async (space: SpaceId, generation = spaceGeneration.current) => {
      ...
      const [menuRaw, snapshot, projects, settings, identity, , counts, entityKinds] = await Promise.all([
         ...
         * (migration 153). It rides this same bounded boot read rather than a
         * lane of its own because the answer is needed before the first list
         * panel draws its kind selector: 153 retired the task `type` axis into
         ...
         * reads that gate boot. Empty is the pre-153 answer exactly — every
  ...
  useEffect(() => {
    ...
    // outgoing space owned (153). Leaving it would let the previous space's
