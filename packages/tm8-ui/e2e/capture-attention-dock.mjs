...
/* Where the four PNGs land: $DOCK_OUT, or a temp directory. It was the
   author's own scratch path, baked in — which is a directory that exists on one
   machine for about a day, and silently fails the run everywhere else. Out of
   the repo rather than into it, because every other harness here writes to
   scratch and nothing in .gitignore expects PNGs under e2e/. */
const SP=process.env.DOCK_OUT ?? await mkdtemp(join(tmpdir(),'tm8-dock-'));
...
await p.screenshot({path:join(SP,'dock-rest.png'),animations:'disabled'});
...
await p.screenshot({path:join(SP,'dock-scrolled.png'),animations:'disabled'});
...
await p.screenshot({path:join(SP,'dock-open.png'),animations:'disabled'});
...
console.log('--- dark ---');
console.log(JSON.stringify(await geom(),null,1));
await p.screenshot({path:join(SP,'dock-dark.png'),animations:'disabled'});
await b.close();
