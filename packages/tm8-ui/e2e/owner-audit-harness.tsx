...
function Harness() {
  ...
  /* AUDIT COPY: the deployed tree no longer exports `ListViewSwitcher`, which
     is why `home-header-harness.html` 500s. The aside is not one of the eight
     surfaces under audit, so this copy renders none. Nothing else differs. */
  const renderRootAside = useCallback((_listRoot: HomeRoot): ReactNode => null, []);
