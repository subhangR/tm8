...
**Status:** FINAL through the second iteration and the fifth-layer correction. Capture HEADs: db18c95 (findings #1–#7), feea2f6 (corrected lifecycle tiers), with the ink-chip parity fix at d877d83 landing after the last capture — hashes in gate-evidence/README.md.
...
## 10. The second iteration: findings #8–#10 and the Run workflow (user-ordered scope)

The user's continued review produced three more findings and two scope directives (D44/D51 — the complete launch-configuration workflow pulled forward from A2). All landed in one extended window:

- **#8 (task panel "missing a lot of details")** — split three ways by measurement: composed-canvas anatomy (metadata grid, subtree, RUNS row, LINKED chips) built as registry-carried blocks; ACCEPTANCE stays A2 under D30 with a measured boundary (the string appears zero times in the composed Z3 region); the Run/Coordinate/Complete primaries were already arriving.
- **#9 ("click run, nothing happens")** — closed at three depths: every unwired verb structurally disabled-with-reason (five verbs were dead, not one — the class, not the instance); the silent optional-dispatch mechanism now throws with both halves tested; and the behavior itself — Run opens quick-config (teammate/model/resolved-profile-with-provenance), Launch dispatches through the seam and a real fixture session appears in the live set. The full T5-5 sheet behind it: five concerns, three sections, trust-gated M:N projects with the honest §10.7 gate on additional associations, T2-4 profile-status honesty, the pinned-at-launch caption asserted to precede commit in DOM order.
- **#10 ("terminal should take full height")** — user-ratified terminal-dominant default: one 26px collapsed line carrying both facts (project-or-absence, share count), sections render only when open, the canvas takes all remaining height, expansion capped at 38%, and the drop target surfaces on dragover regardless of collapse. *Empty sections never budget vertical space away from a live terminal.*

**Laws this iteration minted** (each incident-backed in the ledger/corpus): the three-clause sequencing law (producer-first for additions, consumer-first for deletions, the trigger is always the SHA — an uncommitted change orders nothing); *a capture proves what you point it at*; *a ruling not pinned by a test is a comment*; *an escalation is not a licence to proceed carefully — it is a reason not to proceed*; the two-reads rule (token AND tree, neither implies the other); and D20's epitaph — *the best ending a workaround gets: nothing to unwind, only something to delete* (retired on contract truth the same day the contract landed).

Ledger at close: D1–D56+, including two user-ratified entries (D43, the #10 default), one user scope directive pair (D44/D51), and append-only corrections whose provenance is named in every case.

**The fifth layer — how "the tiers work" was wrong four different green ways.** After the iteration above closed, the lifecycle tiers were still wrong on screen, and the chain of verifications that all passed while they were wrong is the most instructive record in this report. The contract DECLARED a sessionStatus filter (green: schema test). The fixtures held correctly-statused data (green: dataset test). The seam executor, once fixed at 80dc8aa, implemented the filter (green: executor-side test, armed to fire red first). And the screen did not change — because the shell's `rowsFor` declared a filter parameter its implementation ignored: type-legal, invisible to tsc, in the gap between two green suites (panel tests inject their own rowsFor; executor tests call the seam directly; nothing asserted the shell's CALL). Every lifecycle-tiered kind on screen received the same unfiltered array three times over: four sessions displayed as twelve, and Tasks read 9 where the truth was 4 — plausible-looking numbers, which is why nobody noticed until the Sessions counts turned coincidentally symmetric. Fixed at feea2f6 (filter-aware hydration cache per the ruled LLD §3.1 design; a cache miss returns empty, never the unfiltered set — "not loaded yet" and "everything" must never share a value again, and a test pins that), with the red recorded by restoring the broken implementation first. The corrected frame (`sessions-corrected-feea2f6.jpg`) shows disjoint Open/Done sets summing to the true total; the earlier count frame's totals are quarantined in gate-evidence/README.md. D57 gains its amendment: verify the declaration, the data, the implementation, **and that the caller passes it** — an executor never invoked with the filter is indistinguishable, from every green we ran, from one that ignores it.

**The parity directive (user, live review).** During this iteration the user set the acceptance bar plainly: pixel-parity with the T0-1 hi-fi canvas. Two findings are already in motion: primary create chips rendered brand-outline where the canvas renders ink — fixed at d877d83 (ink/paper token pair, canvas-measured hover) — and "letters too small," routed as a spec-level question (the build's type sizes were transcribed verbatim from the canvas, so if the canvas *as presented* reads better, the answer is one root-scale ruling, not per-component tweaks — measurement in flight). The systematic canvas diff (colors → type → spacing, ruled-vs-drift ledger) is running as the next cycle; its output lands as PARITY-LEDGER.md with its own capture set.

**Scope note — the live terminal (Track P).** `src/terminal/` is a master-chartered carve-out owned by a separate autonomous coordinator (Track P), excluded from this report's scope, the FE test globs, and every figure above; its mid-bring-up state is not a gate finding. Two FE-owned files were brokered to support it — the xterm/pty dependencies in `package.json` and the `--pn-x-term-*` theme family in `canvas-extra.css` (commit 6b3fd0f, P-authored, FE-reviewed) — and neither alters any reviewed surface. P's throwaway harness (`scratch-p1-run-src/`, untracked) likewise never appears in any suite count here. The reviewer will see a terminal region in the shell; its interior is reviewed under P's own milestone, not this gate.

## 11. What the R5 review should do
