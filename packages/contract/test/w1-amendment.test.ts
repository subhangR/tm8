...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 127-row target (110 W1 + voice.token.create + 6 artifacts + execution.resume + spaces.counts + execution.journal + identity.profile.update + 4 auth + execution.launch + execution.transcript) without changing reserved honesty', () => {
    ...
    // 126 -> 127 (2026-08-07): execution.transcript (GET read) — what the agent
    // SAID, read from its own native JSONL. Completes told/did/said.
