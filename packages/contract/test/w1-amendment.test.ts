...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 128-row target (110 W1 + voice.token.create + 6 artifacts + execution.resume + spaces.counts + execution.journal + identity.profile.update + 4 auth + execution.launch + projects.directories.list + execution.dispatch) without changing reserved honesty', () => {
    ...
    // 127 -> 128 (2026-08-09): execution.dispatch (POST command) — the ONE new
    // row the dispatcher needs; resolution of "which dispatcher, is it alive"
    // is server-side because every client that has tried has read
    // work_sessions.status and been wrong.
