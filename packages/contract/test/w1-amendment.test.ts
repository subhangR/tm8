...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 156-row target without changing reserved honesty', () => {
    ...
    // 172 -> 173 (chat run controls): chat.threads.interrupt — stopping the
    // turn a chat thread is running. `execution.terminate` could not serve
    // here: it targets a work_session entity and a chat thread has none in its
    // path. MEASURED per PIN RULE v3, never carried.
    ...
    // chat run controls: POST 80->81 — chat.threads.interrupt. MEASURED.
    ...
    // chat run controls: command 106->107 — chat.threads.interrupt. MEASURED.
