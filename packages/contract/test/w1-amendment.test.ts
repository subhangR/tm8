...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 132-row target, including per-user git credentials, without changing reserved honesty', () => {
    ...
    expect(OPERATIONS).toHaveLength(132);
    expect(V1_OPERATIONS).toHaveLength(130);
    ...
    expect({
      ...
      WS: count('method', 'WS'),
    }).toEqual({ GET: 49, POST: 56, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 });
    expect({
      read: count('kind', 'read'),
      command: count('kind', 'command'),
      stream: count('kind', 'stream'),
    }).toEqual({ read: 52, command: 79, stream: 1 });
  });
});
