...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 138-row target without changing reserved honesty', () => {
    ...
    // (POST commands) — browser-originated folder import materialized as a
    // linked project (R7); manifest frozen at init, bytes per raw grant,
    // complete reconstructs beneath a server-authorized destination.
    ...
    expect({
      ...
      WS: count('method', 'WS'),
    }).toEqual({ GET: 52, POST: 62, PATCH: 10, DELETE: 9, PUT: 7, WS: 1 });
    expect({
      read: count('kind', 'read'),
      command: count('kind', 'command'),
      stream: count('kind', 'stream'),
    }).toEqual({ read: 55, command: 85, stream: 1 });
  });
});
