...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 156-row target without changing reserved honesty', () => {
    ...
    expect(OPERATIONS).toHaveLength(223); /* +1 events.changes (change feed step 3). MEASURED. */ // +10 credentials.space.* and node.credentials.* (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED.
    expect(V1_OPERATIONS).toHaveLength(221); /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // launch.suggest is v1: 206 -> 207. MEASURED.
    ...
    }).toEqual({ GET: 74, POST: 107, PATCH: 14, DELETE: 14, PUT: 12, WS: 2 }); /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3. MEASURED.
    expect({
    ...
    }).toEqual({ read: 78, command: 143, stream: 2 }); /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3. MEASURED.
  });
});
