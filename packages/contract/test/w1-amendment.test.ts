...
describe('W1 adopted catalog target', () => {
  ...
  it('reconciles the additive 156-row target without changing reserved honesty', () => {
    ...
    }).toEqual({ GET: 88, /* +6 servers.* (W8, 991): GET +2, POST +4. MEASURED. */ /* +7 spaceLinks.* (W6, 250/251): GET +1, POST +5, PATCH +1. MEASURED. */ /* +1 node.metrics.get (status strip). MEASURED. */ /* W11: GET +2 (spaces.projects.list, gate.folders.list), POST +2 creates (projects.link stays, decision 29). MEASURED. */ /* +1/+1 auth.sessions.list/revoke (W4). MEASURED. */ /* +1 auth.space.enter (W3-server). MEASURED. */ /* +1 spaces.chatDefaults.get. MEASURED. */ /* +1 launch.defaults (I9b). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* GET +1 pendingForSessions, POST +1 responses.redeliver (Forms W3). MEASURED. */ POST: 129 /* +3 G6 leave/remove/disable. MEASURED. */, PATCH: 17, DELETE: 17, PUT: 15 /* +1 spaces.chatDefaults.set. MEASURED. */, WS: 2 }); /* +1 PUT entities.header.set, +1 DELETE entities.header.clear (headers I4). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3. MEASURED. /* -1 containers.attention (Attention v2 S7a). MEASURED. */
    expect({
    ...
    }).toEqual({ read: 92, /* +6 servers.* (W8, 991): read +2, command +4. MEASURED. */ /* +7 spaceLinks.* (W6, 250/251): read +1, command +6. MEASURED. */ /* +1 node.metrics.get (status strip). MEASURED. */ /* W11: read +2, command +2 (projects.link stays, decision 29). MEASURED. */ /* +1/+1 auth.sessions.list/revoke (W4). MEASURED. */ /* +1 auth.space.enter (W3-server). MEASURED. */ /* +1 spaces.chatDefaults.get, command +1 spaces.chatDefaults.set. MEASURED. */ /* +1 launch.defaults (I9b). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* read +1, command +1 (Forms W3). MEASURED. */ command: 174 /* +3 G6. MEASURED. */, stream: 2 }); /* +2 entities.header.set/clear (headers I4). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3. MEASURED. /* -1 containers.attention (Attention v2 S7a). MEASURED. */
  });
});
