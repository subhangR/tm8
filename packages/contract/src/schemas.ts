...
export const CreatableEntityKindSchema = z.union([
  // `container` is excluded for the same reason `work_session` is: it is born
  // only from its own family's create, which reserves the record and then
  // provisions a runtime. A generic create would make a container with no
  // machine behind it.
  CoreEntityKindSchema.exclude(['message', 'member', 'work_session', 'project', 'interaction_profile', 'worktree', 'artifact', 'container']),
