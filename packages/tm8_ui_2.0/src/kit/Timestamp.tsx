/**
 * Timestamp — the one way this app renders "when".
 *
 ...
 * `createdAt` for a message, `activityAt`/`updatedAt` for a row that sorts by
 * recency — because no single field is right everywhere.
 *
...
export interface TimestampProps {
  ...
  /** Leading word rendered with the label: 'updated', 'since', 'latest'. */
  prefix?: string;
  className?: string;
  ...
  /** Overrides the accessible name, which otherwise reads as the label. */
  'aria-label'?: string;
}

export function Timestamp({
  at, prefix, className, title, now: pinnedNow, 'aria-label': ariaLabel,
}: TimestampProps) {
  ...
  const label = prefix ? `${prefix} ${relTime(t, now)}` : relTime(t, now);
  return (
    <time
      className={className}
