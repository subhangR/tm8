...
4. ~~**TOC chip background.**~~ **RESOLVED 2026-08-17 — the chip is gone.** The outline is a list, not a row of pills, so there is no chip background left to diverge. The oracle's pill row was reproduced faithfully and lost to USE: at 17 chapters with titles of six to nine words it wrapped across five ragged lines starting at arbitrary x positions, and the entries could not be scanned as a table of contents at any tokening. The oracle frame carries four short chapter names, which is the case a pill row is right for. `--pn-line` now draws a hairline guide down the list instead. *Ruled by the user; the divergence from the canvas is now structural, not a colour.*
5. **The reading column ships no heading style** — ~~markdown headings are promoted into the OUTLINE instead of restated in the column~~ **superseded 2026-07-31**: the column renders markdown, headings included, and `kit/markdown.css` maps them onto the package's `--pn-fs-*` scale. Headings are in BOTH places on purpose — the outline is navigation, the column is the document.
6. ~~**Heading-derived TOC chips are LABELS**~~ **RESOLVED 2026-08-17 — they are controls that arrive.** The reason they were captions was real ("in-document anchoring is not built"), and the fix was to build the anchoring rather than to keep apologising for its absence: `kit/Markdown` marks every rendered heading with `MD_HEADING_ATTR`, keyed by SOURCE LINE so the slug dedup lives in `headingsIn` alone, and the reader jumps by querying that attribute **inside its own body root**. Two hazards are why the pairing is an attribute and not an `id`+`href`: this app is HASH-ROUTED (`#slug` would replace the route), and this body mounts more than once (split panels, and the two-theme test render), so a global id lookup would scroll the wrong copy. R7 is intact — entity entries still go disabled-with-reason without a dispatch; what changed is that heading entries now HAVE one. *Ruled by the user.*
