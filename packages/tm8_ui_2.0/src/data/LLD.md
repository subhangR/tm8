...
| **Provenance**: `authored_from` is permanently null through public writes — a **declared gap** (G2, CHAT-SYSTEM-DESIGN.md:240, build step S2 assigned), not a surprise defect. | Message provenance is **optional in the model**; "from this session" renders hollow until S2 lands. `MessageView` DTOs pass through verbatim; the fixture dataset (C-5) must include null-provenance messages so the gate screen proves the rendering. |
