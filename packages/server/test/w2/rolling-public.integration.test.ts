...
describe.sequential('W2.I02 real production public surface', () => {
  ...
  it('reports the exact 99-handler production composition and preserves 501/404 honesty', async () => {
    ...
    expect(health).toMatchObject({ ok: true, operations: 221, implemented: 219 }); /* +1 events.changes (change feed step 3). MEASURED. */ // +10/+10 SC-3 space/node credential ops. MEASURED. // +3/+3 service keys (Jev lane K). MEASURED. // +1/+1 launch.suggest (Jev lane F #655). MEASURED.
    expect(harness.production.server.registry.size).toBe(219); /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F #655). MEASURED.
    ...
    expect(registered.size + residual.length).toBe(219); /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F #655): registerable v1 HTTP. MEASURED.
