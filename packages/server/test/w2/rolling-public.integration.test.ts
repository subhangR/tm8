...
describe('W2.I02 tranche-v2 public composition', () => {
  ...
  it('declaratively binds every completed tranche command and leaves only unfinished commands unbound', () => {
    ...
    expect(Object.keys(INPUT_SCHEMAS)).toHaveLength(147); /* +1 credentials.space.share (SC-8). MEASURED. */ /* +10 forms.* command schemas (Forms W1). MEASURED. */ // +7 SC-3 space/node credential command schemas. MEASURED. // +2 service-key put/delete (Jev lane K). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
...
describe.sequential('W2.I02 real production public surface', () => {
  ...
  it('reports the exact 99-handler production composition and preserves 501/404 honesty', async () => {
    ...
    expect(health).toMatchObject({ ok: true, /* +13 forms.* (Forms W1). MEASURED. */ operations: 238, implemented: 236 } /* +2 SC-8 share rows. MEASURED off /health. */); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10/+10 SC-3 space/node credential ops. MEASURED. // +3/+3 service keys (Jev lane K). MEASURED. // +1/+1 launch.suggest (Jev lane F #655). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    expect(harness.production.server.registry.size).toBe(236); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F #655). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    ...
    expect(registered.size + residual.length).toBe(236); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F #655): registerable v1 HTTP. MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
