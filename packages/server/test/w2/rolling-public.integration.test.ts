...
/*
 * `TASK_WORKFLOW_NET_NEW_OPERATIONS` (W4/132) STOOD HERE — the three
 * `spaces.taskWorkflows.*` routes, per-`type`-value status vocabularies mounted
 * with that wave. Phase 6 retires them with the `type` axis they keyed on, and
 * every count in this file that summed them corrects itself, because those
 * counts are DERIVED from these group lengths rather than written down. The
 * hard-coded `/health` numbers below are the exception and are re-MEASURED.
 */
...
describe('W2.I02 tranche-v2 public composition', () => {
  ...
  it('replaces G02\'s eight and G04\'s two legacy registrations and mounts the exact 94-operation facade tranche', () => {
    ...
    expect(registry.size).toBe(149); // -3 (155): the spaces.taskWorkflows handlers
  ...
  it('declaratively binds every completed tranche command and leaves only unfinished commands unbound', () => {
    ...
    // 99 -> 97 (155): the taskWorkflows UPSERT and DELETE input schemas leave.
    // Two, not three — `spaces.taskWorkflows.list` was a GET and never had one,
    // which is why this pin moves by a different amount than the route counts.
...
describe.sequential('W2.I02 real production public surface', () => {
  ...
  it('reports the exact 99-handler production composition and preserves 501/404 honesty', async () => {
    ...
    // -3/-3 (155): the three spaces.taskWorkflows routes retire, and all three
    // were mounted, so the catalog and the implemented count fall together.
    // MEASURED off /health.
    expect(health).toMatchObject({ ok: true, operations: 168, implemented: 166 });
    expect(harness.production.server.registry.size).toBe(166); // -3 (155)
    ...
    expect(registered.size + residual.length).toBe(166); // -3 (155): the spaces.taskWorkflows handlers
