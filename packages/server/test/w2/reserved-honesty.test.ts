...
describe('W2.G15 catalog and production-handler accounting', () => {
  ...
  it('keeps the exact 130 = 128 v1 + 2 reserved, 129 HTTP + 1 WS boundary', () => {
    ...
    // 128 -> 130 (2026-08-09): files.browse + files.read, two GET reads — the
    // Files browser over the node's real filesystem (FILES-DESIGN §5.1).
    // Measured off the built catalog, not derived as previous-plus-two.
    ...
    expect(OPERATIONS.filter(({ method }) => method !== 'WS')).toHaveLength(129);
