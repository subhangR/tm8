...
describe('W2.G15 catalog and production-handler accounting', () => {
  ...
  it('keeps the exact 172 = 170 v1 + 2 reserved, 171 HTTP + 1 WS boundary (+3 148)', () => {
    ...
    // +1 (chat run controls): chat.threads.interrupt, one POST command.
    expect(OPERATIONS).toHaveLength(173); // MEASURED
    ...
    )).toHaveLength(170); // +1 (chat run controls): chat.threads.interrupt
  });
