...
describe('W2.G15 catalog and production-handler accounting', () => {
  ...
  it('keeps the exact 198 = 196 v1 + 2 reserved, 196 mounted HTTP + 1 mounted WS boundary (+25 177)', () => {
    ...
    expect(OPERATIONS).toHaveLength(223); // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +25 (177) containers, MEASURED; /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F)
    ...
    expect(V1_OPERATIONS).toHaveLength(221); /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED.
    ...
    expect(OPERATIONS.filter(({ method }) => method !== 'WS')).toHaveLength(221); /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED.
    ...
    )).toHaveLength(219); // +10 SC-3 space/node credential ops; +3 service keys (Jev lane K); +1 launch.suggest (Jev lane F); 169 -> 193 (177): the container handlers; +1 (187); +1 (execution.gitStage); /* +1 events.changes (change feed step 3). MEASURED. */ +9 skills (2026-09-23)
  });
