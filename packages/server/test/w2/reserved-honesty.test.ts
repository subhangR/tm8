...
describe('W2.G15 catalog and production-handler accounting', () => {
  ...
  it('keeps the exact 120 = 118 v1 + 2 reserved, 119 HTTP + 1 WS boundary', () => {
    ...
    expect(OPERATIONS).toHaveLength(120);
    expect(V1_OPERATIONS).toHaveLength(118);
    expect(RESERVED_OPERATIONS.map(({ name }) => name)).toEqual([
      'search.query',
      'bridge.fetchBlob',
    ]);
    expect(OPERATIONS.filter(({ method }) => method !== 'WS')).toHaveLength(119);
