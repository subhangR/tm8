...
describe('W2.G15 catalog and production-handler accounting', () => {
  ...
  it('keeps the exact 159 = 157 v1 + 2 reserved, 158 HTTP + 1 WS boundary', () => {
    ...
    expect(OPERATIONS).toHaveLength(159); // merge union 2026-08-13: chat.threads.start joins tracking.pr.merge, MEASURED
    expect(V1_OPERATIONS).toHaveLength(157);
    expect(RESERVED_OPERATIONS.map(({ name }) => name)).toEqual([
      'search.query',
      'bridge.fetchBlob',
    ]);
    expect(OPERATIONS.filter(({ method }) => method !== 'WS')).toHaveLength(158);
