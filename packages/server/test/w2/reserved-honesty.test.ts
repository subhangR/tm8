...
describe('W2.G15 catalog and production-handler accounting', () => {
  ...
  it('keeps the exact 198 = 196 v1 + 2 reserved, 196 mounted HTTP + 1 mounted WS boundary (+25 177)', () => {
    ...
    expect(OPERATIONS.filter(({ method }) => method !== 'WS')).toHaveLength(246); /* +3 spaces.leave, spaces.members.remove, accounts.disable (G6, 230). MEASURED. */ /* +2 spaces.chatDefaults.get/set (entity chat G). MEASURED. */ /* +1 launch.defaults (I9b). MEASURED. */ /* Forms W3 + headers I4, on the merged tree. MEASURED. */ /* +2 entities.header.set/clear (headers I4). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
