...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 150 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(151);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(151);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(151);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(150);
  ...
  it('BOTH READINGS ARE CORRECT — 150/0 and 149/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 151 - withPresence.size]).toEqual([151, 0]);
    expect([withoutPresence.size, 151 - withoutPresence.size]).toEqual([150, 1]);
  }, 15_000);
