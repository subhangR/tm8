...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 128 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(128);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(128);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(128);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(127);
  ...
  it('BOTH READINGS ARE CORRECT — 128/0 and 127/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 128 - withPresence.size]).toEqual([128, 0]);
    expect([withoutPresence.size, 128 - withoutPresence.size]).toEqual([127, 1]);
  }, 15_000);
