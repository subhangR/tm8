...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 126 v1 HTTP rows, by exact count and exact membership', () => {
    // 118 -> 122 (2026-08-02): the four auth.* rows (Identity v2 Stage 1).
    // 122 -> 123 (2026-08-02): execution.launch.
    // 123 -> 126 (2026-08-04): this pin had also missed projects.directories.list;
    // re-derived with projects.files.list and projects.files.attach.
    // 114 -> 118 (2026-08-01): execution.resume, spaces.counts,
    // execution.journal, identity.profile.update.
    expect(REGISTERABLE).toHaveLength(126);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(126);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(126);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(125);
  ...
  it('BOTH READINGS ARE CORRECT — 126/0 and 125/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 126 - withPresence.size]).toEqual([126, 0]);
    expect([withoutPresence.size, 126 - withoutPresence.size]).toEqual([125, 1]);
  }, 15_000);
