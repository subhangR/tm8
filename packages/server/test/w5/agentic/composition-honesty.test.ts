...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 148 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(148);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(148);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(148);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(147);
  ...
  it('BOTH READINGS ARE CORRECT — 148/0 and 147/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 148 - withPresence.size]).toEqual([148, 0]);
    expect([withoutPresence.size, 148 - withoutPresence.size]).toEqual([147, 1]);
  }, 15_000);
