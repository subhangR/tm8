...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 145 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(145);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(145);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(145);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(144);
  ...
  it('BOTH READINGS ARE CORRECT — 145/0 and 144/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 145 - withPresence.size]).toEqual([145, 0]);
    expect([withoutPresence.size, 145 - withoutPresence.size]).toEqual([144, 1]);
  }, 15_000);
