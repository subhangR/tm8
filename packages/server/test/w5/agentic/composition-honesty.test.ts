...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 151 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(152);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(152);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(152);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(151);
  ...
  it('BOTH READINGS ARE CORRECT — 151/0 and 150/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 152 - withPresence.size]).toEqual([152, 0]);
    expect([withoutPresence.size, 152 - withoutPresence.size]).toEqual([151, 1]);
  }, 15_000);
