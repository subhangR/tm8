...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 161 unconditional v1 HTTP rows, by exact count and exact membership (+3 W4/132)', () => {
    ...
    expect(REGISTERABLE).toHaveLength(204); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(204); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    // branch's execution.gitStage BOTH land, so this moves twice. Git merged
    // the number line silently — only the comment beside it conflicted. MEASURED on the merged tree from this assertion's own failing run.
    expect(withPresence.size).toBe(204); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    // branch's execution.gitStage BOTH land, so this moves twice. Git merged
    // the number line silently — only the comment beside it conflicted. MEASURED on the merged tree from this assertion's own failing run.
    expect(withoutPresence.size).toBe(203); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  ...
  it('BOTH READINGS ARE CORRECT — 191/0 and 190/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 204 - withPresence.size]).toEqual([204, 0]); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    expect([withoutPresence.size, 204 - withoutPresence.size]).toEqual([203, 1]); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */
  }, 15_000);
