...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 114 v1 HTTP rows, by exact count and exact membership', () => {
    expect(REGISTERABLE).toHaveLength(114);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(114);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(114);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(113);
  ...
  it('BOTH READINGS ARE CORRECT — 114/0 and 113/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 114 - withPresence.size]).toEqual([114, 0]);
    expect([withoutPresence.size, 114 - withoutPresence.size]).toEqual([113, 1]);
  }, 15_000);
