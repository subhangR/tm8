...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 122 v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(122);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(122);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(122);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(121);
  ...
  it('BOTH READINGS ARE CORRECT — 122/0 and 121/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 122 - withPresence.size]).toEqual([122, 0]);
    expect([withoutPresence.size, 122 - withoutPresence.size]).toEqual([121, 1]);
  }, 15_000);
