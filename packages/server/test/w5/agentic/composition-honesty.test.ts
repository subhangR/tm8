...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 158 unconditional v1 HTTP rows, by exact count and exact membership (+3 W4/132)', () => {
    ...
    expect(REGISTERABLE).toHaveLength(159);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(159);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(159);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(158);
  ...
  it('BOTH READINGS ARE CORRECT — 159/0 and 158/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 159 - withPresence.size]).toEqual([159, 0]);
    expect([withoutPresence.size, 159 - withoutPresence.size]).toEqual([158, 1]);
  }, 15_000);
