...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 98 v1 HTTP rows, by exact count and exact membership', () => {
    expect(REGISTERABLE).toHaveLength(98);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(98);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(98);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(97);
  ...
  it('BOTH READINGS ARE CORRECT — 98/0 and 97/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 98 - withPresence.size]).toEqual([98, 0]);
    expect([withoutPresence.size, 98 - withoutPresence.size]).toEqual([97, 1]);
  }, 15_000);
