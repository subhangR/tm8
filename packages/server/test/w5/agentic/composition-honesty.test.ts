...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 139 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    expect(REGISTERABLE).toHaveLength(139);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(139);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(139);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(138);
  ...
  it('BOTH READINGS ARE CORRECT — 139/0 and 138/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 139 - withPresence.size]).toEqual([139, 0]);
    expect([withoutPresence.size, 139 - withoutPresence.size]).toEqual([138, 1]);
  }, 15_000);
