...
describe('W5.F composition honesty — the presence source is the whole delta', () => {
  ...
  it('CONTROL — the denominator is the 130 unconditional v1 HTTP rows, by exact count and exact membership', () => {
    ...
    // 130 -> 131 (2026-08-09): projects.files.read, mounted.
    // 131 -> 134 (2026-08-09): projects.folderUploads.init/complete/abort,
    // all three mounted unconditionally alongside the project-file readers.
    expect(REGISTERABLE).toHaveLength(134);
    expect(new Set(REGISTERABLE).size, 'no duplicate names in the denominator').toBe(134);
  ...
  it('KNOWN-GOOD world — WITH a presence source, residual is the EMPTY SET', () => {
    ...
    expect(withPresence.size).toBe(134);
  ...
  it('KNOWN-BAD world — WITHOUT a presence source, residual is EXACTLY [presence.get]', () => {
    ...
    expect(withoutPresence.size).toBe(133);
  ...
  it('BOTH READINGS ARE CORRECT — 130/0 and 129/1 name the two compositions, not a defect', () => {
    ...
    expect([withPresence.size, 134 - withPresence.size]).toEqual([134, 0]);
    expect([withoutPresence.size, 134 - withoutPresence.size]).toEqual([133, 1]);
  }, 15_000);
