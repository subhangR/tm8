...
describe('W5.C schema-valid stub sweep — all 98 v1 non-WS operations', () => {
  ...
  it('sweeps exactly the 136 v1 non-WS operations, derived from the catalog', () => {
    ...
    expect(SURFACE).toHaveLength(265); /* +1 credentials.space.addMine (W10d). MEASURED. */ /* +7 spaceLinks.* (W6, 250/251). MEASURED. */ /* +1 node.metrics.get (status strip, #876). MEASURED: CI run 36228308315 "expected [ …(250) ] to have a length of 249". */ /* W11: +4 (projects.link stays, decision 29) spaces.projects.list|create and gate.folders.list|create. MEASURED. */ /* +1 auth.space.enter (W3-server). MEASURED. */ /* +6 credentials.space.* (W10b). MEASURED. */ /* 241→244 G6 W1-server: spaces.leave, spaces.members.remove, accounts.disable. MEASURED: CI run 36198882819 "expected [ …(244) ] to have a length of 241". */ /* +2 spaces.chatDefaults.get/set (entity chat G). MEASURED. */ /* +1 launch.defaults (I9b). MEASURED. */ /* Forms W3 + headers I4, on the merged tree. MEASURED. */ /* +2 entities.header.set/clear (headers I4). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ // +10 SC-3 space/node credential ops. MEASURED. // +3 service keys (Jev lane K); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +2 auth.sessions.list/revoke (W4). MEASURED. */ /* -1 containers.attention (Attention v2 S7a). MEASURED. */
    ...
    expect(new Set(rows.map((r) => r.op)).size).toBe(265); /* +1 credentials.space.addMine (W10d). MEASURED. */ /* +7 spaceLinks.* (W6, 250/251). MEASURED. */ /* +1 node.metrics.get (status strip, #876). MEASURED: CI run 36228308315 "expected [ …(250) ] to have a length of 249". */ /* W11: +4 (projects.link stays, decision 29) spaces.projects.list|create and gate.folders.list|create. MEASURED. */ /* +1 auth.space.enter (W3-server). MEASURED. */ /* +6 credentials.space.* (W10b). MEASURED. */ /* 241→244 G6 W1-server. MEASURED. */ /* +2 spaces.chatDefaults.get/set (entity chat G). MEASURED. */ /* +1 launch.defaults (I9b). MEASURED. */ /* Forms W3 + headers I4, on the merged tree. MEASURED. */ /* +2 entities.header.set/clear (headers I4). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +2 auth.sessions.list/revoke (W4). MEASURED. */ /* -1 containers.attention (Attention v2 S7a). MEASURED. */
  });
  ...
  it('applies the FULL migration chain, enumerated rather than hand-listed', () => {
    ...
    // 187 -> 188 (change feed step 2, PR #671): 205_workspace_event_subject_ids,
    ...
    expect(server.appliedMigrations.length).toBe(226); // 225 -> 226 (W10c #904): 257_credential_attach_gate (renamed from placeholder 997; set at merge position after #898). Main + own delta: #898's 225 + 1. MEASURED on the merged tree: ls db/migrations/*.sql | wc -l -> 226, duplicate prefixes -> 0. // 224 -> 225 (W7p #898): 256_space_link_provenance (renamed from placeholder 992; set at merge position after #869). Main + own delta: #869's 224 + 1. MEASURED on the merged tree: ls db/migrations/*.sql | wc -l -> 225, duplicate prefixes -> 0. // 223 -> 224: 255_credential_ops (W10b #869; set at merge position after #864; renamed from placeholder 998). Main + own delta: main d8343503's 223 + 1 (base 219, ours 220). MEASURED on the merged tree: ls db/migrations/*.sql | wc -l -> 224, duplicate prefixes -> 0. // 221 -> 223 (W6 #864): 250_space_link_kinds + 251_space_link_tokens. Main + own delta: main ad33322c's 221 (#906) + 2 (base 219, ours 221). MEASURED on the merged head: ls db/migrations/*.sql | wc -l -> 223, duplicate prefixes -> 0. // 220 -> 221: 254_attention_writes_emit_full_upsert (Attention v2 S2, task 01a0de49; set at merge position after #902). MEASURED: ls db/migrations/*.sql | wc -l -> 221, duplicate prefixes -> 0. // 219 -> 220: 252_attention_badges (Attention v2 S1-server, task 01a0ddd1; 250/251 left to WIP #864; set at merge position after #903). MEASURED: git ls-tree HEAD db/migrations/ .sql count -> 220, duplicate prefixes -> 0. // 218 -> 219: 253_chat_authored_delivery_source (#901, merged with this pin unbumped; fix-forward). MEASURED: ls db/migrations/*.sql | wc -l -> 219, duplicate prefixes -> 0. // 217 -> 218: 239_credential_entities (W10a #863; pin at merge position after #861). MEASURED: git ls-tree HEAD db/migrations/ .sql count -> 218, duplicate prefixes -> 0. // 216 -> 217: 247_command_ledger_session_space_pin (W3-audit S3, task 01a0db50; #861, ordinal 247 kept and pin taken at merge position after #857). MEASURED: git ls-tree HEAD db/migrations/ .sql count -> 217, duplicate prefixes -> 0. // 215 -> 216: 249_w4_auth_sessions_list_revoke (W4 #857; set at merge position after #860; sorts after 248). MEASURED: git ls-tree HEAD db/migrations/ .sql count -> 216, duplicate prefixes -> 0. // 214 -> 215: 246_mark_notification_read_space_pin (W3-audit S2, task 01a0db30; train 1 after #854). MEASURED: git ls-tree HEAD db/migrations/ .sql count -> 215, duplicate prefixes -> 0. // 213 -> 214: 236_auth_session_issued_here (Relay, task 01a0da1f; train 1, stacked on #870 e6244553). MEASURED: git ls-tree HEAD db/migrations/ .sql count -> 214, duplicate prefixes -> 0. // 212 -> 213: 248_member_tombstone_restore_join_and_enter (forward fix for #848). MEASURED: git ls-tree --name-only HEAD db/migrations/ | grep -c '\.sql$' -> 213, duplicate prefixes -> 0. // 211 -> 212: 234_space_owned_projects (#845 W11-model, rebased onto main d11e0be5; 235 split out to #865). MEASURED: git ls-tree -r --name-only <rebased tree> db/migrations | grep -c .sql -> 212, duplicate prefixes -> 0. // 210 -> 211: 233_space_enter_and_pin (W3-server, on main bd1841bf). MEASURED: git ls-tree --name-only HEAD db/migrations/ | grep -c '\.sql$' -> 211, duplicate prefixes -> 0. // 209 -> 210: 232_member_tombstone (G6, W1-server; renumbered from 231, which Chat Context took first). MEASURED: ls db/migrations/*.sql | wc -l -> 210, duplicate prefixes -> 0. // 208 -> 209: 231_chat_context (Chat Context; renumbered from 230, which I7 took first). MEASURED: ls db/migrations/*.sql | wc -l -> 209, duplicate prefixes -> 0. // 207 -> 208: 230_jev_calls_references_group (I7). MEASURED: ls db/migrations/*.sql | wc -l -> 208, duplicate prefixes -> 0. // 206 -> 207: 229_space_chat_defaults (entity chat G). MEASURED: ls db/migrations/*.sql | wc -l -> 207. // 205 -> 206: 228_project_link_guards (doc 15 B3/B4, W0c). MEASURED: ls db/migrations/*.sql | wc -l -> 206, duplicate prefixes -> 0. // 203 -> 205: 226_auth_session_space + 227_session_space_pin (W0a). MEASURED: ls db/migrations/*.sql | wc -l -> 205, duplicate prefixes -> 0. // 202 -> 203: 225_analyze_never_analyzed_tables (#807). MEASURED: ls db/migrations/*.sql | wc -l -> 203, duplicate prefixes -> 0. // 201 -> 202: 224_profile_draft_context_keys (I5b #761). MEASURED: ls db/migrations/*.sql | wc -l -> 202. // 200 -> 201: 223_entity_headers_lenient. MEASURED: ls db/migrations/*.sql | wc -l -> 201. // 199 -> 200: 222_forms_pending_viewer_in_space. MEASURED. // 197 -> 199: 220_secdef_membership_once_per_statement (#777, landed unpinned) + 221_forms_redeliver_and_pending (Forms W3). MEASURED. // 196 -> 197: 218_rls_membership_once_per_statement (#770). MEASURED. // 195 -> 196: 216_entity_headers (headers I3). MEASURED. // 194 -> 195: 215_forms_delivery_spawn_modes (Forms W2 spawn modes). MEASURED. // 193 -> 194: 214_forms_delivery (Forms W2). MEASURED. // 192 -> 193: 212_orchestration_flow_edges (Craft produces/consumes). MEASURED. // 191 -> 192: 211_forms_ops (Forms W1). MEASURED. // 189 -> 190: 208_event_subject_ids_canonical_set (change feed step 3); 190 -> 191: 209_forms_foundation (Forms W0). MEASURED.
