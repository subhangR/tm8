...
describe('W5.C schema-valid stub sweep — all 98 v1 non-WS operations', () => {
  ...
  it('sweeps exactly the 136 v1 non-WS operations, derived from the catalog', () => {
    ...
    expect(rows).toHaveLength(236); /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    ...
    expect(new Set(rows.map((r) => r.op)).size).toBe(236); /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ // +10 SC-3. MEASURED. // +3 service keys (Jev lane K); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  });
