...
describe.sequential('147 — entities.status_category', () => {
  beforeAll(async () => {
    ...
    // …then ONE column from 154, for exactly the reason 135 and 147 are applied
    // on top of the pinned chain elsewhere in this directory: current code
    // selects a column a pre-154 schema does not have. `ENTITY_COLUMNS` and
    ...
    //
    // 154 ITSELF CANNOT BE APPLIED, which is what makes this different from the
    ...
    // 154 §1, which itself depends on nothing past `entity_kinds` (001).
    //
    // This is SEMANTICALLY FAITHFUL rather than a convenient lie: `base_kind`
    // is null on every row, and null base_kind IS the pre-154 world — the
