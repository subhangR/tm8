/**
 * ONE CORRECTION PER MEMORY (migration 187), against a real Postgres.
 *
 * A memory is never edited; a better one replaces it and points a `supersedes`
 * edge at what it replaces. Until 187 nothing stopped two people doing that to
...
async function bindApp(client: PoolClient): Promise<void> {
  ...
  await client.query(
    `select set_config('tm8.identity_id',$1,true), set_config('tm8.actor_id','',true),
            set_config('tm8.node_admin','false',true), set_config('tm8.request_id','mem-187-pg',true)`,
    [fixture.identityId],
  );
}
...
describe('what the person is told', () => {
  ...
  it('leaves every other unique violation on this table exactly as it was', async () => {
    ...
    // violation on this table harder to diagnose than it was before 187.
