...
const ids = {
  ...
  personaA: randomUUID(),
  personaB: randomUUID(),
  /** Linked into A AND B before 234 — the double-linked shape. */
  folderF: randomUUID(),
  /** Linked into A only. */
  folderG: randomUUID(),
  chatA: randomUUID(),
...
async function seedOldShape(): Promise<void> {
  await asOwner(async (q) => {
    ...
    // The double link: legal before 234 (the cap was 16 spaces per folder).
    await q.query(
...
describe('234 on a node with a double-linked folder', () => {
  ...
  it('the one-space-per-folder unique index is DEFERRED: building it now fails on this data', async () => {
    const [row] = await database.query<{ ddl: string }>(
  ...
  it('a chat created after 234 is filled with its space\'s project entity by the trigger', async () => {
  ...
  it('a space\'s project entity resolves to the same folder', async () => {
