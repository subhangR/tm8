...
describe.sequential('W3 production-Server public harness', () => {
  ...
  it('starts the real database-backed production composition', async () => {
    ...
    expect(body).toMatchObject({
      ...
      operations: 220, // +10 SC-3 space/node credential ops. MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED.
    });
    ...
    expect(body.implemented).toBe(218); // +10 SC-3 space/node credential ops, MEASURED; +3 service keys (Jev lane K), MEASURED; +1 launch.suggest placeholder (Jev lane F #655), MEASURED; +9 skills.* (2026-09-23), MEASURED; // +24 (177): the container handlers
