...
describe.sequential('W3 production-Server public harness', () => {
  ...
  it('starts the real database-backed production composition', async () => {
    ...
    expect(body).toMatchObject({
      ...
      operations: 240, /* Forms W3 + headers I4, on the merged tree. MEASURED. */ /* +2 entities.header.set/clear (headers I4). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs (task 01a0d350). /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3 space/node credential ops. MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED.
    });
