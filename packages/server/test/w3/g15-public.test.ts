...
describe.sequential('W3.G15 public reserved and residual honesty', () => {
  ...
  it('reports the frozen HTTP catalog and implementation counts from production health', async () => {
    ...
    expect(OPERATIONS).toHaveLength(240); /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F) // +9 skills (2026-09-23) // +25 (177) containers, +1 (187), +1 (gitStage) /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    ...
    expect(OPERATIONS.filter((operation) => operation.method !== 'WS')).toHaveLength(238); /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    expect(health).toMatchObject({
      ok: true,
      server: 'tm8-server',
      // /health.operations counts ROUTES, not catalog rows (WS never mounts).
      operations: 238, /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs (task 01a0d350). /* +1 entities.commands.tick (bug 01a0d2f1). */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3 space/node credential ops // +3 service keys (Jev lane K) // +1 launch.suggest (Jev lane F) // +9 skills (2026-09-23) // +24 (177): the container HTTP rows; +1 (187); +1 (gitStage)
      implemented: 236, /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ // +1 spaces.configs (task 01a0d350). /* +1 entities.commands.tick (bug 01a0d2f1). */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3, mounted with the credential seam // +3 service keys, mounted with the credential seam (Jev lane K) // +1 launch.suggest (Jev lane F, placeholder handler) // +9 skills (2026-09-23) // +24 (177): all registered, all mounted; +1 (187); +1 (gitStage)
    });
  });
  ...
  it('has an EMPTY residual set: no v1 binding refuses an empty probe', () => {
    ...
    // branch's execution.gitStage BOTH land, so this moves twice. Git merged
    // the number line silently — only the comment beside it conflicted. MEASURED on the merged tree from this assertion's own failing run.
    expect(implemented).toHaveLength(231); /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 SC-3 space/node credential ops, +9 skills (2026-09-23), +1 launch.suggest (Jev lane F), +3 service keys (Jev lane K), MEASURED /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  });
