...
describe.sequential('W3.G15 public reserved and residual honesty', () => {
  ...
  it('reports the frozen HTTP catalog and implementation counts from production health', async () => {
    ...
    // all mounted.
    expect(OPERATIONS).toHaveLength(173); // +1 projects.createFromRepo, +3 148 spaces.workflows
    expect(OPERATIONS.filter((operation) => operation.method !== 'WS')).toHaveLength(172); // +1 createFromRepo, +3 148
    expect(health).toMatchObject({
      ok: true,
      server: 'tm8-server',
      // /health.operations counts ROUTES, not catalog rows (WS never mounts).
      operations: 172, // +1 projects.createFromRepo; +3 148: the spaces.workflows routes
      implemented: 170, // +1 projects.createFromRepo; +3 148
    });
  });
  ...
  it('has an EMPTY residual set: no v1 binding refuses an empty probe', () => {
    ...
    expect(implemented).toHaveLength(170);
  });
