...
/* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ const CATALOG_DIGEST = 'sha256:8bbaf9591423a0881001d6b45c74c7cbf1e4d4885ed251e5572d17b5253a2fc4'; /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ // SC-3: +10 space/node credential rows; /* +1 events.changes (change feed step 3). MEASURED. */ Jev lane K: +3 service-key rows
...
describe('G15 reserved and residual honesty, via generated discovery only', () => {
  it('navigates root -> noun -> operation and finds exactly two reserved operations', async () => {
    ...
    expect(root.catalog.total).toBe(240); /* +2 forms.responses.redeliver, forms.pendingForSessions (Forms W3). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F) // +3 141, +3 148, +25 177 containers, +1 187, +1 gitStage, +9 skills (2026-09-23) /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
