...
const CATALOG_DIGEST = 'sha256:5144c9bcc5d0cff45a03ee3c0c86219c489953bab58156e54d8b474856288bc0'; // SC-3: +10 space/node credential rows; Jev lane K: +3 service-key rows
...
describe('G15 reserved and residual honesty, via generated discovery only', () => {
  it('navigates root -> noun -> operation and finds exactly two reserved operations', async () => {
    ...
    expect(root.catalog.total).toBe(222); // SC-3: +10 credentials.space.* / node.credentials.* rows. MEASURED.  +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // +1 launch.suggest (Jev lane F) // +3 141, +3 148, +25 177 containers, +1 187, +1 gitStage, +9 skills (2026-09-23)
