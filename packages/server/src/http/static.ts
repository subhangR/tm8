 ...
 *
 * AM-1/T-D21: there is no desktop shell. `packages/tm8-ui` is a browser app —
 ...
 *
 * THERE IS EXACTLY ONE BUNDLE, and this handler serves it at the root.
 *
 * Between 2026-08-29 and 2026-09-15 there was a second: a `mountPath` option
 * put the Astryx redesign under `/ui-2.0/` on this same origin, so a version
 * switch could be a navigation rather than a second origin with its own
 * sign-in. That package is gone and so is the option. If a second bundle is
 * ever wanted again, the prefix-stripping it needs is a URL concern only — the
 * traversal guard below already compares against the resolved root and does
 * not care how the path arrived.
 */
