...
export const ENTITY_COLUMNS = `
  ...
  e.status_category,
  -- 155. The "extends" link, resolved for THIS row. A kind whose base_kind is
...
export interface EntityRow {
  ...
  space_id: string;
  kind: string;
  /** 155: `entity_kinds.base_kind` — the `extends` link, or null. See `behaviourKindOf`. */
 ...
 * one, else the kind itself. 155's `internal.base_kind_of`, on the read side.
 *
 ...
 *
 * Before 155 the two were the same string for every row in the graph, which is
...
function stateOf(row: EntityRow, ctx: AssemblyContext): EntityState {
  // 155: the BEHAVIOUR kind. A `c:epic` gets the task arm, so `state.kind` is
...
export function capabilitiesOf(row: EntityRow): EntityCapabilities {
  ...
  // 155: the sets are keyed on BEHAVIOUR, so a `c:epic` is editable,
...
export function entityCapabilities(row: EntityRow): EntityCapabilities {
  ...
  // 155: only a BASE-LESS custom kind takes this arm. A kind that extends
