...
export interface W2CredentialSessionsServiceOptions {
  ...
  now?: () => number;
  /**
   * Persist a GitHub credential into 090's string-shaped table.
   *
   * The storage split is by credential SHAPE: file-shaped credentials
   * (anthropic, openai) are indexed by `account_agent_credentials`, while a
   * GitHub token is a STRING and belongs in `account_git_credentials`. The
   * callback remains injected so the service can preserve the honest
   * `connected: true, stored: false` degradation if a rolling-deploy caller has
   * not wired the store yet.
   */
...
export class W2CredentialSessionsService {
  ...
  private async persistProbe(
    ...
    if (probe.provider === 'github') {
      ...
      // token is string-shaped and belongs in 090's table, not
      ...
      // Extraction happens only AFTER `runCredentialProbe` compared the login
      // in hosts.yml with `gh api user`, and in the identical scrubbed env.
      const token = await readGitHubCredentialToken({
        env: entry.env,
        cwd: entry.homeDir,
        ...(this.probeRunner ? { run: this.probeRunner } : {}),
      });
