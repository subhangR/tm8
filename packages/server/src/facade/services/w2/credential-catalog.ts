 ...
 * node that has 093 would be shown a confident "Not connected", and a member
...
export interface W2CredentialCatalogServiceOptions {
  ...
  /**
   * Revoke a GitHub credential from 093's string-shaped table. Optional only
...
export class W2CredentialCatalogService {
  ...
  async status(principal: CredentialPrincipal): Promise<CredentialsStatusView> {
    ...
    for (const row of git.rows) {
      byProvider.set(row.provider, {
        provider: row.provider as CredentialProviderName,
        // 093's table has no status column of its own,
  ...
  /**
   * Read 093's table if it is here, and say plainly when it is not.
   *
   * `to_regclass` rather than a caught error, because catching would also
   * swallow a permission failure and report it as "absent" — and 093's grant is
    ...
    if (provider === 'github') {
      ...
      if (!this.revokeGitCredential) {
        failures.push({
          step: 'revoke',
          reason:
            'the string-shaped credential store (migration 093) is not present on this node, so nothing was revoked',
        });
