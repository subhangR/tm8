...
export const PROFILE_KNOBS: readonly SubjectKnob[] = [
  ...
  { name: 'contextBudgets.memories', summary: 'Bytes of memories injected whole; past it the lowest-ranked collapse into <context_index>. Refused at save if the budgets cannot fit the prompt.', default: '12288 (BYTE_BUDGETS.memoryInjection)', definedIn: 'packages/contract/src/context-budgets.ts', anchor: 'contextBudgets.memories', change: 'profile' },
  { name: 'contextBudgets.skills', summary: 'Bytes of <context_index> skill entries; unset, skills take what the prompt has left. Refused at save if the budgets cannot fit the prompt.', default: 'what remains', definedIn: 'packages/contract/src/context-budgets.ts', anchor: 'contextBudgets.skills', change: 'profile' },
  { name: 'contextBudgets.references', summary: 'Bytes of <context_index> reference entries (a worker\'s linked teammates share it unless teammates is set). Refused at save if the budgets cannot fit the prompt.', default: '8192 (BYTE_BUDGETS.referenceIndex)', definedIn: 'packages/contract/src/context-budgets.ts', anchor: 'contextBudgets.references', change: 'profile' },
  { name: 'contextBudgets.teammates', summary: 'Bytes of <context_index> teammate entries. Refused at save if the budgets cannot fit the prompt.', default: 'shares references (worker); 8192 (BYTE_BUDGETS.rosterIndex, dispatcher)', definedIn: 'packages/contract/src/context-budgets.ts', anchor: 'contextBudgets.teammates', change: 'profile' },
