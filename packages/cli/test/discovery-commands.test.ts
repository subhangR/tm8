...
describe('the registry is composed from per-noun modules, and agrees with the projection', () => {
  ...
  it('the projection holds 159 command paths; the registry is an honest subset of them', () => {
    ...
    expect(COMMAND_PATHS).toHaveLength(215); /* +16 tm8 form commands: 13 catalog rows + close|cancel|reopen aliases (Forms W1 CLI). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ // +1 events.changes (change feed step 3). MEASURED. // +7 tm8 skill scan|list|show|create|edit|equip|unequip (2026-09-23). MEASURED.
