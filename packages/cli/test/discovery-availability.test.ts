...
describe('the default is unknown, and unknown is never upgraded', () => {
  it('an untouched ledger answers unknown for every non-reserved row', () => {
    ...
    expect(rows).toHaveLength(238); /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ // +10 credentials.space.* + node.credentials.* (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +9 skills.* rows (2026-09-23, #647 + #649); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F). MEASURED.
...
describe('/health is a cache-invalidation EPOCH, never a per-operation claim', () => {
  ...
  it('the epoch NEVER becomes a per-operation claim: 28 of 101 names no operation', () => {
    ...
    expect(rows).toHaveLength(236); /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ // +10 credentials.space.* + node.credentials.* (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +9 skills.* rows (2026-09-23, #647 + #649); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F). MEASURED.
  });
