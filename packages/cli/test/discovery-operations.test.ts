...
describe('the exposure histogram is the one the catalog freeze specifies', () => {
  it('152 public, 1 composite, 1 internal, 2 reserved', () => {
    ...
    expect(histogram).toEqual({ public: 152, composite: 1, internal: 1, reserved: 2 });
  });
});
...
describe('the CLI command projection', () => {
  ...
  it('every command path is 2 or 3 tokens — the closed grammar depth', () => {
    ...
    // Minus the TWENTY-THREE commandless rows named exactly in the test above.
    expect(counted).toBe(EXPECTED_ROWS - 23);
  });
