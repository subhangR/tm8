...
describe('the exposure histogram is the one the catalog freeze specifies', () => {
  it('122 public, 1 composite, 1 internal, 2 reserved', () => {
    ...
    expect(histogram).toEqual({ public: 128, composite: 1, internal: 1, reserved: 2 });
  });
});
...
describe('the CLI command projection', () => {
  it('exactly two rows have no CLI command, and they are the asymmetric pair', () => {
    ...
    expect(commandless.sort()).toEqual([
      'bridge.fetchBlob',
      'execution.prompt',
      // A token on a command line lands in shell history, in the process
      // table of a box running other people's agents, and verbatim in that
      // session's own command journal on disk. These three are entered in the
      // UI or not at all.
  ...
  it('every command path is 2 or 3 tokens — the closed grammar depth', () => {
    ...
    expect(counted).toBe(EXPECTED_ROWS - 8);
  });
