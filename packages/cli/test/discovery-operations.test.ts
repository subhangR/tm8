...
describe('cross-check: the projection agrees with the W1 conformance manifest', () => {
  it('sweeps all 155 manifest help rows and agrees on noun and exposure', () => {
...
describe('the exposure histogram is the one the catalog freeze specifies', () => {
  it('151 public, 1 composite, 1 internal, 2 reserved', () => {
    ...
    expect(histogram).toEqual({ public: 151, composite: 1, internal: 1, reserved: 2 });
  });
});
...
describe('the CLI command projection', () => {
  ...
  it('every command path is 2 or 3 tokens — the closed grammar depth', () => {
    ...
    // Minus the TWENTY-TWO commandless rows named exactly in the test above.
    expect(counted).toBe(EXPECTED_ROWS - 22);
  });
