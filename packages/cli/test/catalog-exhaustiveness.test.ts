...
const EXPECTED_ROWS = 212; // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED.
...
describe('the catalog itself is the shape W4 was briefed on', () => {
  it('199 rows = 197 v1 + 2 reserved, 197 mounted HTTP + 1 mounted WS (measured; +1 187 sharing, +1 execution.gitStage)', () => {
    ...
    expect(V1_OPERATIONS.length).toBe(210); // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED.
    ...
    expect(MOUNTED_OPERATIONS.filter((o) => o.method !== 'WS')).toHaveLength(210); // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED.
  });
});
...
describe('every row resolves through the client and the error mapping', () => {
  ...
  it('an honest 501 on EVERY row maps to exit 8 — and a WS row never reaches the wire', async () => {
    ...
    expect([...resolved.values()].filter((c) => c === 8)).toHaveLength(210); // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED.
    ...
    expect(requested).toHaveLength(210); // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED.
  });
  ...
  it('a success on EVERY row is returned, not mistaken for drift', async () => {
    ...
    expect(httpRows).toBe(210); // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED.
  });
});
