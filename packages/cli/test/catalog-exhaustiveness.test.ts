...
describe('the catalog itself is the shape W4 was briefed on', () => {
  it('199 rows = 197 v1 + 2 reserved, 197 mounted HTTP + 1 mounted WS (measured; +1 187 sharing, +1 execution.gitStage)', () => {
    ...
    expect(MOUNTED_OPERATIONS.filter((o) => o.method !== 'WS')).toHaveLength(238); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  });
});
...
describe('every row resolves through the client and the error mapping', () => {
  ...
  it('an honest 501 on EVERY row maps to exit 8 — and a WS row never reaches the wire', async () => {
    ...
    expect([...resolved.values()].filter((c) => c === 8)).toHaveLength(238); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
    ...
    expect(requested).toHaveLength(238); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED. /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */
  });
  ...
  it('a success on EVERY row is returned, not mistaken for drift', async () => {
    ...
    expect(httpRows).toBe(238); /* +2 credentials.space.share + credentials.shares.list (SC-8). MEASURED. */ /* +13 forms.* (Forms W1). MEASURED. */ /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ /* +1 events.changes (change feed step 3). MEASURED. */ // +10 credentials.space.* + node.credentials.* (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F, 2026-09-23). MEASURED.
  });
});
