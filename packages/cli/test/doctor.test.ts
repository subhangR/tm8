...
describe('doctor registers as a LOCAL command, never as a catalog operation', () => {
  ...
  it('is ABSENT from the operation catalog, and the catalog count is unmoved', () => {
    ...
    expect(COMMAND_PATHS).toHaveLength(199); /* +1 spaces.configs (task 01a0d350). MEASURED. */ /* +1 entities.commands.tick (bug 01a0d2f1). MEASURED. */ // +1 events.changes (change feed step 3). MEASURED. // +7 tm8 skill commands (2026-09-23). MEASURED.
  });
