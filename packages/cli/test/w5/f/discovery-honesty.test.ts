...
describe('W5.F PIN 1 — availabilitySource names a source that did not answer', () => {
  ...
  it('BLAST RADIUS — 128 of 130 rows carry the unearned label on a cold ledger', () => {
    const fresh = new AvailabilityLedger();
    const rows = discovery(fresh);
    expect(rows).toHaveLength(130); // +2 collection membership writes.
    ...
    expect(unearned).toHaveLength(128);
