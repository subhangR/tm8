...
describe('W5.F PIN 1 (CONVERTED) — availabilitySource names a source that produced it', () => {
  ...
  it('BLAST RADIUS, CONVERTED — the two populations are now distinguishable', () => {
    ...
    expect(rows).toHaveLength(169); // +3 148 (spaces.workflows); -3 155 (spaces.taskWorkflows)
    ...
    expect(unknownRows).toHaveLength(167) /* 167 -> 170 (148): the three spaces.workflows rows, non-reserved ; 170 -> 167 (155): the three taskWorkflows rows leave */;
