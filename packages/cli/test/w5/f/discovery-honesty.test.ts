...
describe('W5.F PIN 1 (CONVERTED) — availabilitySource names a source that produced it', () => {
  ...
  it('BLAST RADIUS, CONVERTED — the two populations are now distinguishable', () => {
    ...
    expect(rows).toHaveLength(223); /* +1 events.changes (change feed step 3). MEASURED. */ // +10 credentials.space.* + node.credentials.* (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +1 launch.suggest (Jev lane F). MEASURED. // +9 skills.* rows (2026-09-23, #647 + #649). MEASURED from CI's failing run on main.
    ...
    expect(unknownRows).toHaveLength(221); // +10 (SC-3). MEASURED. // +3 credentials.serviceKeys.* (Jev lane K). MEASURED. // // +9 skills (2026-09-23); /* +1 events.changes (change feed step 3). MEASURED. */ +1 launch.suggest (Jev lane F). MEASURED.
