...
describe('W5.F PIN 1 — availabilitySource names a source that did not answer', () => {
  ...
  it('BLAST RADIUS — 123 of 125 rows carry the unearned label on a cold ledger', () => {
    const fresh = new AvailabilityLedger();
    const rows = discovery(fresh);
    expect(rows).toHaveLength(125); // 121 -> 125 (2026-08-02): auth.* Identity v2 Stage 1 (4 ops, all public, all with commands).
    ...
    expect(unearned).toHaveLength(123);
