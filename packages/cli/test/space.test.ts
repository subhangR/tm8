...
describe('every row binds its path from the catalog', () => {
  it('sends the exact method and bound path for all 22 rows', async () => {
    ...
    expect(checked).toBe(27); // +3 (148): workflow list|set|delete; -3 (153): task-workflow list|set|delete
...
describe('mutation identity (§7.4)', () => {
  it('every read REFUSES --mutation-id and never reaches the network', async () => {
    ...
    expect(checked).toBe(12); // +1 (118): `space invite resolve`; -1 (153): task-workflow list
  });
