...
it('starts a real Server and reports an un-enveloped /health', async () => {
  ...
  // 126 -> 127 (2026-08-09): execution.dispatch. NOTE this counts ROUTES, not
  // catalog rows — the catalog is 128, of which `events.subscribe` is WS and
  // never becomes an HTTP route.
  expect(h.operations).toBe(127);
