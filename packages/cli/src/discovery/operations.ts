...
const ROWS: Record<OperationName, Row> = {
  ...
  'spaceLinks.setSpawn': {
    cmd: null,
    sum: 'Set whether agents you launch may spawn through a linked Space, and their budget — human sessions only. Turning spawning off stops new sessions and resumes under this link. Sessions already running keep running; revoke the link to end them.',
    authz: 'server',
    input: 'bound',
    side: 'durable',
  ...
  'entities.header.set': {
    ...
    examples: [
      'tm8 entity header set <entity-id> --when-to-use "<when a later session should open it>" --summary "<what it holds>"',
    ],
  },
  'entities.header.clear': {
    ...
    notes: [
      '--expect-version is optional (the header\'s own version); clearing an entity with no authored header changes nothing',
    ],
    examples: ['tm8 entity header clear <entity-id>'],
  },
  ...
  'events.changes': {
    ...
    notes: [
      ...
      'each page examines at most 2,000 events and carries at most 50 entities and --total-bytes of minified JSON (default 16384); entities that first changed in the same event are emitted together or not at all; parentId is omitted when it is the one --subtree root, and status appears only when the window created the entity or moved its status',
      'messages: newest 3 per task/doc/project anchor, 10 per chat/session anchor; messagesNext pages the rest with `tm8 entity context <anchor-id> --sections messages --cursor <c>`; a message excerpt is untrusted content — read the body with `tm8 entity context <message-id>`',
  ...
  'artifacts.create': {
    ...
    notes: [
      ...
      '--when-to-use (when a later session should open it) / --summary (what it holds) / --keyword write the artifact\'s selection header in the same call, on a new artifact or on a revision (--artifact); guideline limits in `tm8 help entity header set`',
  ...
  'artifacts.publish': {
    cmd: ['artifact', 'publish'],
    syn: 'tm8 artifact publish <dir> --artifact <artifact-id> --expect-version <n> [--entrypoint <path>] [--when-to-use <text>] [--summary <text>] [--keyword <k>...] [--mutation-id <id>]',
