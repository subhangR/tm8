...
> **⚠ SUPERSEDED IN PART — 2026-08-14, by migration `120`.** The *cap* in the paragraph above is gone: no number of consecutive Teammate reservations is refused, and nothing writes `automated_wake_limit`. Everything else in it still holds — the reservation is still row-locked under one durable unordered pair, the key still contains no thread root, and the three capability decisions are still independent. `SESSION-COMMUNICATION-MODEL.md` §10 carries the full note.
