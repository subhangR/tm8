...
1. **The blueprint already exists outside the old code.** The Collab V2 corpus — entity-graph design, approved API contract (`docs/history/collab-v2/api-design/`), UI data contract, interactive prototype, and the in-flight mock-data UI build — is a complete spec. Most rewrites die because the spec lives in the legacy code; here it lives in documents.
...
- `docs/history/collab-v2/api-design/00..05` (on main) — the API contract: layering, resource grammar, consumer surfaces, communication model, coherence matrix. Read with the substitution *maestro-server → tm8-server* and the auth delta in `05-DECISIONS.md` (T-D3).
- `docs/history/collab-v2/ENTITY-GRAPH-DESIGN.md`, `COLLAB_V2_UI_UX_BRIEF.md`, `GAPS-AND-EXTENSIONS.md`, `UI-DATA-CONTRACT.md` (branch `feat/collab-v2-supabase-backend`) — the data model, UI thesis, deferred slots.
- `docs/history/collab-v2/ui-plan/01,02` (main) — the UI implementation in flight (Atlas, waves W0–W5), which becomes tm8-ui by transplant.
...
The API contract (operations, DTOs, events, errors — inherited from `docs/history/collab-v2/api-design/`) is database-agnostic and transport-independent; the reference implementation is Postgres and uses it fully (triggers, recursive CTEs, GIN, RLS, uuidv7). Locally, Postgres runs as a **bundled sidecar** managed by tm8-server (PGlite is the watched fallback; a SQLite port is rejected). One schema, one migration sequence, laptop and hub.
...
Inherited normative docs: `ENTITY-GRAPH-DESIGN.md`, `GAPS-AND-EXTENSIONS.md`, `docs/history/collab-v2/api-design/01-DATA-MODEL.md` (final table catalog + migration sequence).
...
Read, in order: `docs/architecture/00–06` (this set) → `docs/history/collab-v2/api-design/00–05` (main) → branch docs via `git show feat/collab-v2-supabase-backend:docs/history/collab-v2/ENTITY-GRAPH-DESIGN.md` (+ `_GAPS_AND_EXTENSIONS`, `_UI_UX_BRIEF`, `_UI_DATA_CONTRACT`) → `docs/history/collab-v2/ui-plan/01,02`.
...
**Charter:** `06-SEQUENCING-AND-REVIEW.md` §5. Corpus reviewed: tm8 docs 00–06; `docs/history/collab-v2/api-design/00–05`; branch docs (`COLLAB_V2_ENTITY_GRAPH_DESIGN`, `_GAPS_AND_EXTENSIONS`, `_UI_UX_BRIEF`, `_UI_DATA_CONTRACT` @ `feat/collab-v2-supabase-backend`); `docs/history/collab-v2/ui-plan/01–02`; plus a source-level audit of the maestro-server / maestro-cli execution code against the 04 inventory.
...
**Normative inputs:** tm8 laws + decisions (01/05), the api-design contract (`docs/history/collab-v2/api-design/`, read with tm8-server substitution + R2 auth delta), the execution inventory (04), and every R-condition from 07 (traceability table in §8).
