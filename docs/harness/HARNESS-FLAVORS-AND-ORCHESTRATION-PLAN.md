...
**Status:** design proposal for W1+; no implementation authority. Subordinate to the authority order in `FINAL-DESIGN-SET.md` §1.
**Date:** 2026-07-26
**Inputs:**
- `AGENT-HARNESS-AND-COMMAND-DISCOVERY.md` (W0-adopted harness design — the baseline this plan tiers)
- `CLI-GRAMMAR-REDESIGN.md` rev 4 (the verb surface every journey below is written in)
- `AGENT-JOURNEY-WALKTHROUGH.md` (the walkthrough both reviewers read)
...
| F8 | ~~`automated_wake_limit` error envelope gains an explicit `nextAction` naming the inbox fallback~~ **MOOT — 2026-08-14, migration `120`.** The breaker this was to make legible no longer exists; nothing writes `automated_wake_limit`. The underlying finding (an agent cannot tell a stuck child from a thinking one) is untouched and still needs the `idle` + no-activity ladder from gap (d) — it just no longer terminates in a wake refusal. | convergent finding 6 |
