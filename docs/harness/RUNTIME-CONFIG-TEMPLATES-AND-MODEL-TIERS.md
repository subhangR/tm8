...
**Date:** 2026-07-27
**Subordinate to:** `WORKSPACE-LAYOUT-AND-TERMINOLOGY.md` v2.11 (RULING L), `CHAT-UI-AND-LAYOUT-DESIGN.md` (§9.6, §14, §15), `AGENT-HARNESS-AND-COMMAND-DISCOVERY.md`, `CLI-GRAMMAR-REDESIGN.md`. Nothing here reopens a closed ruling; §4 proves it.
...
A static template is a **typed, versioned, deploy-validated registry asset** shipped with the Server/UI binary — never an entity, never CLI-authorable, never agent-authored (`WORKSPACE-LAYOUT-AND-TERMINOLOGY.md:22` RULING L; `CHAT-UI-AND-LAYOUT-DESIGN.md` §1.6, §23). Its entire payload is the parameterization below. It reaches the browser only inside the profile's sanitized presentation projection (`AGENT-HARNESS-AND-COMMAND-DISCOVERY.md:203`); there is no template read API family.
...
1. **Immutability:** `(key, version)` is immutable once shipped; any change ships as a new version. The registry retains old versions while any live pin references them — "Registry versions cannot disappear while referenced by a live pin; removal requires an audited pin migration" (`CLI-GRAMMAR-REDESIGN.md:1129`).
...
5. **Selection:** only `interaction_profile.template {key, version}` selects a template (RULING L). Teammates "may select an already shipped static template version, but cannot author templates" (`CLI-GRAMMAR-REDESIGN.md:768`).
...
- The restricted-kind admission rule grants a named command family "only when universal create/patch is refused and its invariant-preserving lifecycle cannot fit universal CRUD" (`CLI-GRAMMAR-REDESIGN.md:746`). Tier invariants — ordered non-empty list, valid names, delete-guard — **fit universal CRUD**, so `model_tier` is an **ordinary core entity kind** with:
  ...
  - **delete guard:** delete refused while any `uses_model_tier` edge references it (`invariant_violation`/`model_tier_in_use` + referencing IDs) — the `profile_default_in_use` pattern (`CLI-GRAMMAR-REDESIGN.md:770`);
...
| CLI | Universal grammar covers reads/writes (`entity get/query/versions`, gated create/patch/delete). `teammate update` gains `--model-tier <id|none>` (writes the edge). No dedicated noun yet (OPEN-4). Tier delete joins the `--yes` confirmation list (`CLI-GRAMMAR-REDESIGN.md:936` family) |
