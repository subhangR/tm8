...
1. `docs/features/memory/MEMORY-AND-STALENESS-DESIGN.md` ("main design")
2. `docs/features/memory/MEMORY-STALENESS-API-CLI-DESIGN.md` ("API doc")
...
- **GRAMMAR:308's wording ("validated before the entity is inserted") is not what ships.** Edges are
  ...
  next bullet (`CLI-GRAMMAR-REDESIGN.md:309-310`) says the then-current `attachTo` **could not**
...
| Handoff §23.15 door-binding rule carried, nothing weakens it | **CONFIRMED for create; GAP for patch** | §23.15 (`W0-W5-HANDOFF-STATE.md:2688-2760`) measured eleven doors on the `entities.create` label; migration 036 shipped all eleven bound plus two repairs (`036:49-70`). Main design §3.1+§6 and API §1.3 all require the 036 pattern on `create_memory`/`create_worktree` from day one — carried, not weakened. But **the same handoff records `entities.patch`'s eleven doors as UNBOUND**, and the designs add doors to that label without a word — F2 |
...
**R2 [minor, high irony].** Main design §0's specimen: "*`PROGRAM-CLOSE.md:28` says fourteen; the
...
`W0-W5-HANDOFF-STATE.md:2733-2736`). The designs' day-one 036-binding rule is stated for create
