...
**Status:** built — `chat-home/induced-graph.ts` + `graph-seeds.ts` +
`use-induced-connections.ts` + `induced-layout.ts` + `ChatEntityGraph.tsx`,
mounted in `ChatHomeScreen` in place of `LiveGraphStrip`. Open questions §9
answered in the implementing PR.
