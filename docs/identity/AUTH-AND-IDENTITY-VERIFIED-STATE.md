...
From `docs/architecture/01-LAWS.md`:
...
`docs/architecture/10-SECURITY-MODEL.md` opens with the framing that matters: *"tm8 v1 is a browser-controlled arbitrary-code-execution system."*
...
Proven working (per `docs/remote/REMOTE-STATUS-2026-07-29.md`): two real Servers on `127.0.0.1:4710` and `:4720` with separate databases; through A's `remote-b` connection the CLI created a Space, Teammate and ProjectResource on B, linked the project, and spawned a work session that ran on B.
...
- **No Firebase and no Supabase**, correctly — T-D3 forbids them in tm8. (`docs/history/collab-v2/crib-supabase/` is reference material, not a dependency.)
...
`docs/remote/REMOTE-END-TO-END-DESIGN.md` (773 lines, 2026-07-27, status: **design draft, for adversarial review**) closes the 15 wire-spec items in `docs/remote/PHASE-2-REMOTE-SERVER-INTEGRATION.md` §11. Highlights:
...
**Docs** — `docs/architecture/`: `01-LAWS.md` `02-NODE-AND-GATEWAY.md` `10-SECURITY-MODEL.md` · `docs/remote/`: `PHASE-2-REMOTE-SERVER-INTEGRATION.md` (binding boundary) `REMOTE-END-TO-END-DESIGN.md` (773-line draft) `REMOTE-STATUS-2026-07-29.md` (audit)
