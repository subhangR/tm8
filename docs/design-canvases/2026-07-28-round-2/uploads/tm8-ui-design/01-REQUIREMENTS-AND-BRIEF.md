...
Chat is a graph-backed peer surface inside the work-session panel (bubbles for explicit messages, cards for artifacts, activity rows — never a parsed terminal transcript). It is fully specified in the repo (`docs/chat-and-messaging/CHAT-UI-AND-LAYOUT-DESIGN.md`) but **sequenced after the whole app runs and the terminal works properly**. For now:
