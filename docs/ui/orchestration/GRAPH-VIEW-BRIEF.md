...
- Design system: ATELIER — `docs/design-canvases/2026-07-28-round-2/uploads/tm8-ui-design/03-DESIGN-LANGUAGE.md` + `05-DESIGN-SYSTEM/tokens.css`. The built gate screen runs at http://localhost:4612 (look at it).
- The canvas suite: `docs/design-canvases/2026-07-28-round-2/*.dc.html` — the approved design language in action. Z1 chip / Z2 card / Z3 panel / Z4 full view.
- The standing graph-view requirements (from `docs/design-canvases/2026-07-27-round-1/DESIGN-REQUESTS-ROUND-2.md`, T3-6 — previously deferred, NOW REACTIVATED by the user): Z2 cards as nodes (reuse EntityCard anatomy, don't invent a node visual), typed edges with direction+label, C1 holds (click any node → its Z3 panel on the stack), focus/dependency modes (center an entity, n-hop neighborhood), cluster rendering, pan/zoom with keyboard access, honesty at scale (what renders at 500+ nodes — honest cap, never a frozen canvas), works both as a workspace-center panel body and as a full view, light+dark.
...
- Old mechanics reference (working code, xyflow+dagre): `docs/design-canvases/2026-07-28-round-2/uploads/tm8-ui-design/07-CURRENT-CODE/collection-layouts/graph/` (GraphCanvas.tsx, EdgeComposer.tsx, model.ts) and `packages/ui/src/collab-v2` graph view.
...
- Read-only against the tree except your ideas doc: write it to `docs/ui/orchestration/GRAPH-VIEW-IDEAS.md`. No git commits (the repo runs a lock ceremony you're not part of; the master lands docs).
