...
- `docs/history/collab-v2/UI-DATA-CONTRACT.md` — DTOs, route matrix, command matrix, events
- `docs/history/collab-v2/ENTITY-GRAPH-DESIGN.md` — domain semantics (Supabase-era)
- `docs/history/collab-v2/GAPS-AND-EXTENSIONS.md` — Parts A/B/C/D backlog
- `docs/history/collab-v2/ui-plan/01-IMPLEMENTATION-PLAN.md`, `02-ORCHESTRATION-PLAN.md`
- `docs/history/collab-v2/api-design/00..05`
- `docs/architecture/00-VISION`, `01-LAWS`, `02-NODE-AND-GATEWAY`,
...
- Event union: `UI-DATA-CONTRACT.md:267-275` — `entity.upsert|deleted`,
...
`ui-plan/02:121-123`'s ownership map, not in `UI-DATA-CONTRACT.md`'s route matrix
...
(`ENTITY-GRAPH-DESIGN.md:433-437`, three layers: User → Member → TeamMember).
...
- `UI-DATA-CONTRACT.md:198`: `GET /v2/spaces/:spaceId/settings` → *"space
...
`UI-DATA-CONTRACT.md:188` gives it `spaces.list` + `spaces.navigation`.
...
`ENTITY-GRAPH-DESIGN.md:486-487`; `01-LAWS.md:25` files invites under
*"operational"* side tables.

**Roles are specified** — `ENTITY-GRAPH-DESIGN.md:697`: *"**Role gates:** space
...
| **Member management actions** | Roles are listed for *display*; no design for change-role, remove-member, transfer-ownership. `ENTITY-GRAPH-DESIGN.md:697` names "member removal" as a role-gated action with no surface. |
...
- **Channel hub Docs tab** (brief §7) surfaces attached docs; files ride the same
  `attached_to` registry expansion (`UI-DATA-CONTRACT.md:246-257`).
...
|---|---|
| `UI-DATA-CONTRACT.md:26` — *"Reads are scoped by the **authenticated Firebase user** and RLS"* | T-D3 (`05-DECISIONS.md:9`): *"No Firebase, no Supabase, anywhere in tm8"* |
...
| `ENTITY-GRAPH-DESIGN.md` throughout (`:46, :98, :106-107, :133, :701-703`) — Supabase Storage, `auth.users`, RTDB, FCM | T-D3 |
