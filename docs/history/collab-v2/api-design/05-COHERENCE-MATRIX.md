...
**Part of:** `docs/history/collab-v2/api-design/` — see `00-OVERVIEW.md`.
**Answers:** proof that every UI surface, CLI command, and agent workflow maps onto the operation catalog (`02 §4`) — and the exact, enumerated deviations from `UI-DATA-CONTRACT.md` with a migration note each (the UI mock facade must follow these).
...
## 3. Deviations from `UI-DATA-CONTRACT.md`
