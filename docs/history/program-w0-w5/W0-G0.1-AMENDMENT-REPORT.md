...
The baseline matrix remains a name-set bijection with the current catalog. The additive A01–A20 names and bindings remain identical between dossier and matrix. `W0-CONSISTENCY-MATRICES.md` is byte-for-byte unchanged because neither A20 metadata nor any other catalog/kind disposition changed.

## 6. Exact documentation scope

Changed only:

- `W0-AMENDMENT-DOSSIER.md` — the exact storage, view, writer, lock, revision, backfill, RLS/rollback rules and three-RPC wording;
- `W0-GATE-REPORT.md` — G0.1 evidence/binding and current W1 pause;
- `W0-W5-HANDOFF-STATE.md` — G0.1 entry condition and preserved W1 resume boundary;
- `FINAL-DESIGN-SET.md` — authority-index status and G0.1 evidence link;
- this report.
