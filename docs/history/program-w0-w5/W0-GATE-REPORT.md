...
- `docs/history/program-w0-w5/W0-W5-HANDOFF-STATE.md`
- `docs/history/program-w0-w5/W0-G0.1-AMENDMENT-REPORT.md` (W0-E)

Amended canonical authorities:

- `FINAL-DESIGN-SET.md`
- `FINAL-DESIGN-SET-REVIEW.md`
...
- `docs/architecture/05-DECISIONS.md`

No file under `packages/`, `db/migrations/`, UI source, or Remote Phase 2 was edited.

## 2. Closure outcome

`FINAL-DESIGN-SET-REVIEW.md` §7 records each B1/B2, M1–M7, and m1–m13 disposition and exact owning text. Highlights:
...
awk -F'|' '$2 ~ /^[[:space:]]*[0-9]+[[:space:]]*$/ {n++; h=$6; gsub(/[[:space:]]/,"",h); if(h=="F") f++; if(h ~ /^X/) x++; if(h=="E") e++; if(h=="WS") ws++; if(h=="R") rr++} END {print n,f,x,e,ws,rr,f+x+e}' docs/history/program-w0-w5/W0-CONSISTENCY-MATRICES.md
```
...
```sh
rg -n 'thread_root_message_id|threadRoot,unordered|Review-ready design|pending Vega adoption|execution\.prompt.*user-authored' docs
rg -n 'use_message_send|automated_wake_limit|session_wake_budgets|canMessage|canContactSession|canHandoffEntity' docs docs/architecture/05-DECISIONS.md
rg -n 'TM8_AUTH_TOKEN|TM8_AGENT_TOKEN|message send --anchor|--reply-to' docs/harness/AGENT-HARNESS-AND-COMMAND-DISCOVERY.md
rg -n 'scope=default|direct_v1|session_chat_v1|message_batch_id|Default-channel amendment' docs
rg -n -i 'now (introduces|recompute|takes|inherit)|Scratch execution has|The lease survives|markWarm fires|verified holding|correctly removes' docs/architecture/WORKSPACE-LAYOUT-REVIEW.md
rg -n '^## (16|17|18|19)\. Round' docs/architecture/WORKSPACE-LAYOUT-AND-TERMINOLOGY.md
```
...
fa2c304a5ee24ee7c5d9eb47e157c38eb8d5aa6145b8a4a99046ae0a21f11c60  W0-CONSISTENCY-MATRICES.md (unchanged G0/G0.1)
```
