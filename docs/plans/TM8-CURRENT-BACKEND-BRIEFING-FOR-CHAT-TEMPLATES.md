...
The conceptual tm8-server is one Server domain made from six composable blocks: graph/contract, Postgres, HTTP/WS server and identity, execution/PTY, bridge, and gateway (`docs/tm8-architecture/01-LAWS.md:7`). Phase 1 has real packages for:
...
The graph/state and terminal streams are separate by law: durable session state lives in the graph; live terminal bytes stay out of Postgres (`docs/tm8-architecture/01-LAWS.md:66`).
...
- Human grouped guide, mechanically reconciled to all 81 rows: `docs/plans/TM8-API-CATALOG-GROUPED-GUIDE.md`.
...
- `docs/plans/TM8-CLI-GRAMMAR-REDESIGN.md` revision 4: graph/domain noun-first grammar and an exact 81-operation disposition table.
- `docs/plans/TM8-SESSION-COMMUNICATION-MODEL.md` revision 4: proposed durable message-first live-session delivery, teammate inboxes, provenance and guarded attachments.
- `docs/plans/TM8-AGENT-HARNESS-AND-COMMAND-DISCOVERY.md`: the agent journey, orchestration harness, progressive command discovery, native provider launch, and explicitly deferred structured-provider seam.
