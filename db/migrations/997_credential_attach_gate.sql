-- =============================================================================
-- 997 (PLACEHOLDER NUMBER) — the private-credential attach gate, W10c (task
-- 01a0da8c, doc 13 01a0da24 §3h / §6a T4, threat review 01a0db1c R2, R9, R16).
--
-- Runs after 996_credential_entities (W10a) and reads its columns. The merge
-- coordinator assigns the real number at handover (order a, c, b from 239);
-- this file is renamed then and the sweep.test.ts pin re-measured.
--
...
--   3. consume_stream_attach (087's body): a grant is consumed only if its
--      SUBJECT still passes the rule. A grant minted while the card was public
...
-- -----------------------------------------------------------------------------
-- 3. Consuming a grant. 087's body; the UPDATE also requires the grant's
--    SUBJECT to pass the rule, so every failure still converges on one refusal.
...
begin
  ...
  -- consumed row or private credential was the part that failed to match.
  ...
  update public.stream_grants g
   ...
   where g.work_session_id = p_session_id
     ...
     and (claim_identity is null or g.subject_identity = claim_identity)
     and not exists (
       select 1
         from public.session_space_credentials ssc
         join public.space_credentials sc
        ...
        where ssc.work_session_id = p_session_id
          ...
          and not exists (
            select 1 from public.accounts a
             where a.id = sc.owner_account_id
               and a.status = 'active'
               and a.identity_id = g.subject_identity))
  returning g.* into consumed;
...
--    private credential the caller does not own. It says nothing about
--    membership or the dials: the journal and transcript handlers keep their
--    RLS entity read as the membership gate, and the PTY recheck applies it to
--    sockets that already passed grant_stream_attach.
-- -----------------------------------------------------------------------------
...
comment on function public.session_stream_credential_allowed(uuid) is
  'False iff the session records a private space credential whose owner is not the '
  'caller''s account (997, doc 13 §3h). Read paths outside grant_stream_attach use it.';
