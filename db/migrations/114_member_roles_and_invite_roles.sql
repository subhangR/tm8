-- 114 — PERMISSIONS YOU CAN ACTUALLY CHANGE: member roles, invite roles, and a
...
comment on column public.space_invites.role is
  'The space role this invite confers on redemption (114). Admits admin and '
...
comment on function public.set_member_role(uuid, uuid, text, uuid, text) is
  'Change a member''s space role (114). Space admin required; only an owner may '
...
comment on function public.preview_invite(text) is
  ...
  '(114). Claim-free by necessity — the reader is not a member of the space the '
