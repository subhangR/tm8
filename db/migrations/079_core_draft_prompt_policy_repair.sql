...
--
-- NUMBERED 079, NOT 077: this was authored as 077 to sit after this lane's own
-- 078 (`derived_from` props_schema). Main has since taken 076 and 077, so both
-- of this lane's files renumbered on landing.
-- `w2-migration-order.pg.test.ts:70` requires only that the three-digit
-- prefixes be UNIQUE and sorted — never contiguous. Landing LAST in the chain
-- is what this file wants: it is a repair of `internal.w2g12_core_draft()`, and
-- it must be the final word on that function's body.
--
...
comment on function internal.w2g12_core_draft() is
  ...
  'InteractionProfileDraftSchema; 079 checks the first at apply time after 072 '
