...
--
-- NUMBERED 079 because 078 in this same change is the derived_from repair, and
-- main's chain ended at 077. Authored on origin/feat/per-user-private-workspaces
-- and brought here unchanged except for this paragraph; that branch is unmerged,
-- so the defect below is live on main. `w2-migration-order.pg.test.ts:70`
-- requires only that the three-digit prefixes be UNIQUE and sorted — never
-- contiguous. See 078's header for the number-contention note.
--
