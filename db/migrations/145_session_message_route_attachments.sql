...
-- `[{fileEntityId,name,mime}]` manifest on EACH message copy, then asks this
-- function which target sessions are owed those copies. The live definition
-- returned the body but omitted the adjacent `messages.attachments` column.
-- Every caller therefore received a delivery-shaped object that silently
-- claimed no files existed.
--
-- WHY THIS FIELD BELONGS HERE, rather than in a TypeScript side map built only
-- by `messages.post`. This SECURITY DEFINER function is the single convergence
-- point for all four route producers: work-session anchors, replies, explicit
-- pokes, and task workers. The forge watcher is already a second caller and a
-- third caller must not have to rediscover a parallel enrichment rule. A field
-- on the canonical route is total by construction; a caller-owned map is an
-- optional convention whose honest-looking failure mode is `count="0"`.
--
-- SECURITY DEFINER DOES NOT WIDEN THE ATTACHMENT AUDIENCE. The caller cannot
-- name an arbitrary message: the batch guard below requires every id to belong
-- to the current authored command and binds its author to the current actor.
-- Before that command stored the manifest, 019's
-- `internal.w2_validate_attachment_files` required each file to be live,
-- finalized, in the message Space, readable by the posting identity, and
-- compatible with every anchor's visibility. This function then derives a
-- TARGET SESSION that is owed that stored message copy. Returning the copy's
-- own already-validated manifest to the delivery path is therefore the right
-- authority question: "what is this recipient owed?", not "can the posting
-- viewer re-read it during a second projection?" The manifest is identity
-- metadata only; file bytes still require the target agent's own file read.
--
-- WHAT DOES NOT CHANGE. The stored attachment edges and file authorization are
-- unchanged. `messages.attachments.add/remove` still do not create a new
-- delivery; mutating an already-delivered message is a separate notification
-- contract. The route set, reply provenance, prompt-policy pin, and all four
-- 121 target classes below are copied verbatim.
--
-- GENERATED by copying the complete live 121 definition forward, then adding
-- exactly one `attachments` projection beside `body`. Never edit 121: deployed
-- databases have already recorded it. The owner caveat in 121's header remains
-- load-bearing, so this migration likewise deliberately has no `set role`.
-- =============================================================================
