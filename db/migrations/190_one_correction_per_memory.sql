-- =============================================================================
-- 190 — ONE CORRECTION PER MEMORY.
--
-- Authored as 187, which was the next free number on main when this work
-- forked from it, and renumbered once on the way in. feat/agentic-memory had
-- already moved its own three files up to 187/188/189 to get out of the way of
-- main's 185 and 186, so 187 was taken by the time these two lanes met. The
-- incoming side moves: this file is one rename and four comment references,
-- while moving the other three again would rewrite a dozen ledger notes for
-- the third time.
--
-- Renumbering is free here for the usual reason — this file adds a new index
-- and replaces a function by name, it derives nothing from its own number, and
-- the suite that covers it resolves the chain by running every file in order
...
--    verbatim, with the insert wrapped in the marked `-- 190:` handler and
...
declare
  ...
  -- 190: the caught violation, and the rival correction it is about. All four
...
begin
  ...
  -- 190: the insert, and only the insert, runs inside a handler. Everything
