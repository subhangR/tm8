-- =============================================================================
-- 155  KIND ABSORBS THE TYPE AXIS.
--
...
begin
  ...
  -- 155: SAME BASE KIND, not the same kind. The literal comparison this
...
comment on function internal.validate_entity_parent() is
  'Hierarchy is same-BASE-kind as of 155 (was same-kind since 001). Space, '
...
begin
  ...
  -- 155: a detail row belongs to an entity whose BASE kind is this table's kind.
...
begin
  ...
  if found then
    -- 155: an endpoint satisfies the registry by its own kind OR by its base.
...
begin
  ...
  -- 155: the kind OR its base satisfies the expectation.
...
comment on function internal.workflow_for_entity(uuid, text) is
  ...
  'then the core row), else THE built-in default. 155 removed the per-type arm '
...
begin
  ...
  -- 155: 150 read `tasks.axes ->> 'type'` here and threaded it into the
...
begin
  ...
  -- 155: the type-value lookup that stood here is gone with the axis.
  wf_id := internal.workflow_for_entity(e.space_id, e.kind);
...
comment on function internal.workflow_kind_is_executable(text) is
  ...
  'for `task` and for every kind that extends it. 155 widened it from the '
...
begin
  ...
  -- 155: the door accepts `task` and any kind of this space that EXTENDS task.
  ...
  -- 150: the workflow's INITIAL state, not the literal 'open'. 155: resolved
...
begin
  ...
  -- 155: base kind, so a `c:epic` completes through the same door and the same
...
begin
  ...
  case p_intent
    ...
    when 'assign' then
      -- 155: base kind on both arms. An epic is assignable because it is a task.
    ...
    when 'subtask', 'reparent' then
      -- 155: SAME BASE KIND, matching internal.validate_entity_parent exactly.
...
begin
  ...
  perform internal.require_space_admin(p_space_id);
  actor := internal.resolve_actor(p_actor_id, p_space_id);
  -- 155: `label` and `labelPlural` join the patch key whitelist. `baseKind` does
...
begin
  ...
  -- 155: BASE kind, and the reported `sourceKind` is the row's own kind rather
...
begin
  ...
  raise notice '155: % kinds created, % tasks re-kinded', kinds_created, tasks_rekinded;
end
$migrate$;
...
begin
  ...
  -- 155: the `type` axis seed was here. A new space gets no taxonomy axis;
...
comment on constraint task_axes_type_is_a_kind on public.task_axes is
  '155. `type` is not an axis — it is the kind. Axes remain for honest tags '
  '(team, quarter).';
...
begin
  ...
  if bad > 0 then raise exception '155 verify: % type axis rows survived', bad; end if;
  select count(*) into bad from public.tasks where axes ? 'type';
  if bad > 0 then raise exception '155 verify: % tasks still carry axes.type', bad; end if;
  ...
  if bad > 0 then raise exception '155 verify: % malformed base_kind rows', bad; end if;
  ...
  if bad > 0 then
    raise exception '155 verify: % entities of a task-based kind have no tasks row', bad;
  end if;
  ...
  if bad > 0 then
    raise exception '155 verify: % migrated entities hold a status outside their kind''s workflow', bad;
  end if;

  raise notice '155 verify: ok';
end
$verify$;
