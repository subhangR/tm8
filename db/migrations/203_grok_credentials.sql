-- =============================================================================
-- 203 — GROK (xAI) JOINS THE ADMITTED PROVIDER SET.
--
-- RENUMBERED 197 -> 203 (2026-09-23). This file first ran on Utho prod as
-- `197_grok_credentials.sql` from a build that never reached main, and main
-- then gave 197 to the skills migration. Prod's ledger therefore still lists
-- the old name, and this file WILL apply there a second time. That is safe:
-- every statement below is `create or replace` or drop-then-add of a CHECK
-- whose admitted list only grows, so a re-run on a database that already
-- holds a 'grok' row leaves it exactly as it was.
--
