-- 136 -- `craft`, the 6th chat mode (Craft P1, task 01a00a31, 2026-08-16).
-- Style-only like every mode (PR #265 ruling: modes state intent, never
-- permission) -- the prompt variant lives in server chat/compose.ts; this
-- migration only widens the two places the DATABASE enumerates modes.
--
-- Same forward-only constraint swap as 124 (which added `explain`), and the
-- same function-level twin: this is the 124 `start_chat_thread` definition
-- verbatim (verified: no later migration redefines it) with only the accepted
-- mode set widened.
...
alter table public.chat_threads
  add constraint chat_threads_chat_mode_check_v3
  check (chat_mode in ('ask', 'explain', 'plan', 'build', 'orchestrate', 'craft')) not valid;

alter table public.chat_threads
  validate constraint chat_threads_chat_mode_check_v3;

alter table public.chat_threads
  drop constraint chat_threads_chat_mode_check;

alter table public.chat_threads
  rename constraint chat_threads_chat_mode_check_v3 to chat_threads_chat_mode_check;

-- Keep the function-level rejection aligned with the table constraint. This
-- is the latest 124 definition with only the accepted mode set widened.
