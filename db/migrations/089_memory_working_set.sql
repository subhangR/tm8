-- =============================================================================
-- 089 — Memory working set: move `team_members.memories` jsonb into 056 memory
...
--   089 (this file)  — copy every jsonb entry into a memory entity + a
  ...
  loop
    ...
    loop
      ...
      values (
        memory_id,
        stmt,
        'unrecorded — migrated from the legacy team_members.memories jsonb column (089)',
      ...
      values (tm.space_id, tm.entity_id, memory_id, 'remembers',
              jsonb_build_object('note', 'migrated from team_members.memories (089)'),
              tm.entity_id)
