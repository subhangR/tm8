...
if [[ "$BUILD_UI" == 1 ]]; then
  ...
  #
  # ONE BUNDLE SINCE 2026-09-06, when the second UI package was deleted:
  # packages/tm8-ui is the product UI at `/`, and the only one.
  #
  # STALE SYMLINKS FIRST. tm8-ui/dist has pointed at ../tm8_ui_2.0/dist on this
  # box, while a root-owned env named the old path; a symlink there makes a vite
  # build write THROUGH it into a package that no longer exists. Remove it
  # before building; idempotent, and a no-op on a box that never had one.
  ...
  # The second-UI pointers name packages that no longer exist. The server has
  # stopped reading them, so they are inert rather than dangerous — but an env
  # file that names a deleted directory is exactly the kind of thing a future
  # reader trusts. Strip them here, where we are root anyway; idempotent.
  if grep -qE '^TM8_UI_(1_0|2_0)_DIR=' "$ENVFILE"; then
    sed -i -E '/^TM8_UI_(1_0|2_0)_DIR=/d' "$ENVFILE"
    rok "removed the stale TM8_UI_*_DIR pointers from $ENVFILE"
  fi
else
