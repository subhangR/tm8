#!/usr/bin/env bash
# tm8 PROD UI — 7777, `vite preview` over packages/tm8-ui/redesign-1.0.
#
...
if [[ ! -f "$TM8_PROD_ROOT/packages/tm8-ui/redesign-1.0/index.html" ]]; then
  echo "run-ui.sh: no UI bundle at $TM8_PROD_ROOT/packages/tm8-ui/redesign-1.0 — run deploy/prod/deploy.sh" >&2
  exit 1
fi
