...
# A stale `packages/tm8-ui/dist` SYMLINK (it used to point at the now-deleted
# ../tm8_ui_2.0/dist, bridging a root-owned /etc/tm8/prod.env that named the old
# path) would make the product build below write THROUGH it. Remove it first;
# idempotent, and a no-op on a tree that never had one.
