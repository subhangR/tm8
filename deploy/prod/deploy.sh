...
# A stale `packages/tm8-ui/dist` SYMLINK (it used to point into the deleted
# alternate UI package, bridging a root-owned /etc/tm8/prod.env that named the
# old path) would make the build below write THROUGH it, out of this package.
# Remove it first; idempotent, and a no-op on a tree that never had one.
